...
tags: [GEODESIC, ADR-003, flow matching, rectified flow, velocity head, straightness, G-FLOW-STRAIGHTNESS, G-F3-CAPTURE, FM drafter, steering head, do not rebuild, agent, tier-2]
mem_tier: full
---
...
sp_status: DESIGN
sp_gate: "pre-flight chain: G-F3-CAPTURE → G-FLOW-STRAIGHTNESS (tiered decision) — no head trained before both are GREEN"
sp_commit: "proposal v2 (supersedes 9fbf0f2) — engine untouched until the pre-flight chain is GREEN"
sp_repro: "F3 pairs do NOT yet exist on disk (verified 2026-07-03: no *capture* dirs / capture_feat artifacts in any repo, depth-3 sweep). First executable step = the §5 capture."
---
...
**Verified 2026-07-03: the F3 pairs do NOT exist on disk** (depth-3 sweep of all five
repos: no `*capture*` dirs, no `capture_feat` artifacts). UNIFICATION §"Near-term
execution" already scoped the capture; this section makes it buildable.
