...
**DONE this session:** GIT HYGIENE (submodule divergence RESOLVED — submodule+standalone both at b1fae37; engine N0 `a503eae`+N1a `68cd7ff` pushed). **N1a loader GREEN** (gate G_DG_N1 26/26, `sp_model_to_diffusion_gemma` bridge loads the diffusion-gemma .sp-model).

1. **★ N5a — CUDA MoE forward (the prerequisite, GPU, free now).** DISCOVERY at N1a: `cuda_forward.cu` is **dense-only — NO MoE on the GPU path**; the MoE forward exists ONLY in CPU `qwen36.c`. DiffusionGemma is 128-expert/8-used MoE, so this MUST come before N1b. Port the MoE forward into `cuda_forward.cu`: F32 router GEMV → top-8 softmax/renorm → per-expert OK_Q4B dp4a over the FUSED `ffn_gate_up_exps[2816,1408,128]` + `ffn_down_exps[704,2816,128]` → weighted-sum. Gate **G-DG-N5a**: byte-exact (top-1 + small rel-err) vs the CPU `qwen36.c` MoE on a single block. Reference the CPU `qwen36.c` MoE (`gemma4_build_ffn_moe` analog) + the existing dp4a GEMV path.
2. **N1b — region-aware `[prompt|canvas]` overlay (after N5a).** Small overlay behind `SP_DIFFUSION`: split P=n_tok-canvas; prompt query=causal SWA-clipped / canvas query=bidirectional; canvas embed=rmsnorm_noscale(embed*sqrt d); per-layer enc/dec scalar split (enc_layer_output_scale vs layer_scalar). No self-cond at step 0. Gate **G-DG-N1b**: logits bit-close to the PR-24423 oracle (`D:\F\llama-diffgemma-pr24423`; extract oracle logits via the diffusion-gemma-eval target or a debug dump in that SEPARATE clone). Then N2(folds in)→N3(self-cond)→N4(entropy sampler)→N5b(hetero GPU/CPU expert split)→N6(native judge)→N7(drafter). Full plan: `DESIGN-diffgemma-native-port.md` (sequencing corrected).
3. **ENV FLAG:** C: pagefile was switched fixed-16GB→system-managed during the N0 OOM workaround (reboot-pending); revert if desired. The G-DG-N0 .sp-model is at `C:\sp_models\diffusiongemma-26B-A4B.sp-model` (13GB, out-of-tree, not committed).
