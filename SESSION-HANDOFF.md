...
- **OOD diffusion kill-test (`G-DIFFJUDGE-OOD-H2H`) — RAN, but the oracle run is INVALID (re-run needed).** The 26B llama.cpp oracle finished (`OOD_DIFF_DONE_0`) and the harness *printed* 94.4% recall / 98.0% reject "GATE GREEN" — **but it MALFUNCTIONED: every reply was the llama warning `'…W init: embeddings required'`, ZERO `_TAGPOOL` tags in the whole log → the 94.4% is a scoring ARTIFACT, not a real judge.** The run-health caveat caught a FALSE POSITIVE (taking it at face value would have justified N5b on an oracle that judged nothing). **W_c floor stands: K=8 = 28.3% recall / 96.9% reject** (W_c is an in-distribution MEMORIZER — full-registry OOD only 11.1%). Kill-criterion (pinned BEFORE result, `papers/CONTRACT-DIFFJUDGE-OOD-H2H.md` §4): diffusion must beat W_c **+10pp recall AND ≥96.9% reject** → else Phase-5 native lane (incl. N5b) RETIRED. **NEXT SESSION: FIX the oracle invocation (the `'embeddings required'` warning — diff vs the valid G-DIFFJUDGE-1 95.6% call that DID emit tags; likely the `llama-diffusion-gemma-eval` binary or a flag, not `llama-diffusion-cli`), re-run, CONFIRM real tags in the replies, THEN apply §4. The Phase-5 fork stays OPEN until then.**
