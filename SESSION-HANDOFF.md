...
- **XBAR P3.3 SP_REPLAY = CLOSED GREEN (2026-06-17).** Replay-injection seam at the CUDA cache-store boundary. **G-P3-SHARED 3-leg PASS on BOTH 12B (48 owners) + E2B (15 owners / 20 sharers, owner-indirection): intact bit-exact (diffs=0), zeroed diverges 12/12** (collapse to degenerate loop — maximal divergence, leg decisive without a logit probe). Fix that closed the prior WIP: `gemma4_decode_cuda` has graph + velocity prefill stores; gate runs velocity (`use_graph` false under recall/replay) — inject now at BOTH (graph ~2516 + velocity ~2825, owner-only, `wslot`-aware). Engine committed; contract §P3.3 run-record landed. Receipts `_p33_ep{,_e2b}/G-P3-SHARED*.log`.
- **XBAR P3.4 = CLOSED GREEN (2026-06-17). `G-P3-PPL` SP_REPLAY-injection PPL deflection on 12B-b1 (`wiki.tiny` n_ctx=84): recall-OFF 4.6665 → recall-ON (SP_REPLAY proven episode, NPOS=4) 4.7311 = +1.38% < 2.0%.** The PPL scorer IS `gemma4_decode_cuda` in `SP_G4_SCORE` mode, so `SP_REPLAY` composed with zero new engine code. Foreign episode over 4 earliest of 84 positions; model holds focus. Complements the b-2b sparse-recall deflection (+0.47% learned-LSH 8×). Caveat: n_scored=42 single chunk (deterministic, not noise — but larger-N is the hardening lever pre-public). Receipt `_p34_gate/G-P3-PPL_GREEN.log`, runner `_run_p34_ppl.bat`. **→ P3 (ring-on-Exec) CLOSED end-to-end (P3.0→P3.4).** NEXT = the orchestration / Ring-3 consolidation tier ABOVE P3 (Memo curator recall-cue→episode-id→SP_REPLAY loop; Ring-3 gist via the budget-gated P2.b adapter). Detail: memory `project_p33_sp_replay_wip`.
