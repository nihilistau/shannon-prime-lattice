...
1. **★ N4 + native-judge SELECTION eval (the meaningful close).** Build the entropy-bound denoising sampler (`diffusion-sampling.cu` reference) + the constrained `{tags,NULL}` canvas selection on `diffusion_gemma_forward_cuda`, then run the div-corpus judge through OUR native forward and check it reproduces G-DIFFJUDGE-1's recall (95.6%/96.0%). Selection fidelity is robust to the quant delta (real judge inputs have large argmax margins). THIS is the real gate, not byte-exact logits.
