...
**CURRENT EDGE (2026-06-17) — the pivot is DONE, back on XBAR:** **XBAR P3 is CLOSED end-to-end (P3.0 manifest → P3.1 read → P3.2 spill/page/SWA-ring → P3.2-b-2b learned-LSH select → Phase C O(1) VRAM → C-c NIAH → P3.3 replay-write → P3.4 recall-quality).** P3.3 (`SP_REPLAY` `G-P3-SHARED` 3-leg PASS on 12B + E2B: intact bit-exact, zeroed diverges 12/12) and P3.4 (`G-P3-PPL` recall deflection **+1.38% < 2.0% gate**; n=42 single-chunk caveat — larger-N multi-chunk is the named hardening lever) are both CLOSED GREEN. The crossbar reads, writes, compresses to O(1), retrieves under poison, replays bit-exactly, and recalls without breaking the model's perplexity. On the audio axis, the **GNA "EAR" line is CLOSED end-to-end on PHYSICAL SILICON**: real speech → 12B pivot 7/8 (CTC recovery 0.44→0.868); the front-end lowered to GNA-native i16 at full FP32 token-recovery (POT 0.877) and run on the real Intel **GNA 2.0** in the NUC "Beast Canyon" (GNA_HW 0.877 == SW_EXACT emu == FP32). KAIROS time/agency axis remains CLOSED (KAI-1/1b/1c + `run_kairos_metal` crucible + 6h soak GREEN); KAI-2 CLOSED-BOUNDED; KAI-3 CLOSED GREEN. **FRAMING (load-bearing):** KAIROS = the latent-interrupt / agency-time axis = the BASIS OF the XBAR latent memory; the GNA "EAR" line is a **separate-but-related sibling** sharing the inject seam (now physically realized). The near-term audio pivot is DONE; **the project has pivoted BACK to XBAR**. **NEXT = the Memo curator / Ring-3 orchestration tier ABOVE P3** — the autonomous recall loop (recall-cue → episode-id → `SP_REPLAY`), Ring-3 gist consolidation budget-gated via the P2.b adapter. (The PROVEN/OPEN lists below are the older XBAR-campaign snapshot — still accurate, but read the edge above first.)
