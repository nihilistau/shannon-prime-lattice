...
Last rewritten: 2026-06-09 (XBAR campaign; C1-lite complete; P2.b recall operating-point decided). Current-state addendum 2026-06-17 — see §2.
...
**CURRENT EDGE (2026-06-17) — XBAR full stack CLOSED end-to-end:** **XBAR P3 CLOSED** (P3.0→P3.4; SP_REPLAY G-P3-SHARED 3-leg PASS on 12B + E2B; G-P3-PPL +1.38% < 2% gate; n=42 single-chunk caveat — larger-N multi-chunk is the named hardening lever). **C2 Memo curator CLOSED** (Steps 1–3.1 + #222 + G-XBAR-ORGANISM step 1): curator indexes via 256-bit discrete hash (r=256, XOR+popcount, TAU_BITS=168), is bit-inert when off (G-MEMO-NULL 4.6665 bit-identical, engine `3ea0587`), promotes matched recall at +0.000% and flags corrupted recall at +40106.6% (G-MEMO-LOOP, engine `627dfad`). #222 ports SP_REPLAY into the persistent `gemma4_kv_*` ABI (G-222 + G-222-WRAP GREEN, engine `b4b037a`/`24071bc`). G-XBAR-ORGANISM step 1: ep_audio [48,114,512] uniform-512, sig self 211/256 margin +79, SP_REPLAY clean; +1989% deflection is foreign-by-design (audio KV into wiki context), ~0% is matched-context only (engine `6600cf4`). **Ring-3 Path A CLOSED** (R3.1–R3.4 GREEN, parameter-free, zero budget): VSA/HRR NTT bind over Z_q seeded from real C2 256-bit sigs; R3.1 recall@1=1.0 / capacity recall@5≥0.90 to N=64 @ D=1024 (engine `23539b7`); R3.2 hit +0.000% / miss +8.04% gate-caught (engine `aae3131`); R3.3 clean hit + decoy scan + null parity DUALROUTE (engine `69638cf`); R3.4 NIGHTSHIFT D=128 seal proves seal is the math (seals at [10,6,15,8,1], max 15 < CAP=32) (engine `a64a916`). VSA retrieve is host-numpy; Z_q/NTT engine port is deferred deployment. **GNA EAR CLOSED on physical silicon** (GNA_HW 0.877 == emu == FP32). **Caveat on record (period-8-vs-6):** C2/Ring-3 pipeline uses PERIOD=8 (L%8==7); 12B's true SWA period = 6 (L%6==5); all prior gates STAND; period-6 rebase is the next correctness tidy-up. **NEXT: period-6 rebase (correctness), then full G-XBAR-ORGANISM loop** (cue→shortlist→#222 scan→promote). (The PROVEN/OPEN lists below are the older XBAR-campaign snapshot — still accurate, but read the edge above first.)
