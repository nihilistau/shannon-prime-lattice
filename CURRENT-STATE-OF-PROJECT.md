---
type: project-state
title: Shannon-Prime — Current State of the Project
description: "As of 2026-06-20 — B3-WC AUTONOMOUS-RECALL CAMPAIGN RESOLVED: a learned W_c head now does LIVE, instance-level episodic recall on the served Gemma-4-12B chat, with clean foreign-reject (§11; engine ed"
tags: [project-state]
timestamp: 2026-06-19T17:30:13Z
resource: ./CURRENT-STATE-OF-PROJECT.md
sp_status: ACTIVE
sp_gate: none
sp_commit: TBD
sp_repro: none
...
**As of 2026-06-24 -- diffusion-judge whole-machine perf ~2x SHIPPED (scratch-reuse default-on `e31c70d` + async pinned double-buffer byte-exact `2a1c830`; a self-cond-OOB recall regression is IN FLIGHT so the async default-on promotion is HELD) + the Cola block-causal latent-diffusion NORTH-STAR doc landed (`papers/DESIGN-COLA-DLM-MAPPING.md`: block-causal EXPLAINS the prefix-KV refutation, but is a train-time property not a mask flip). See SESSION-HANDOFF section 0-NOW. Prior 2026-06-20 — B3-WC AUTONOMOUS-RECALL CAMPAIGN RESOLVED: a learned W_c head now does LIVE, instance-level episodic recall on the served Gemma-4-12B chat, with clean foreign-reject (§11; engine `edc8079`). Prior (2026-06-18): the XBAR memory architecture UNIFIED onto the exact-integer O_K substrate — Q(√−163), the algebra the project is built on; ten receipts, all GREEN or honest-negative; engine `0019b86→d2d7ceb`. SAME DAY: the BYTE-EXACT FORWARD CLOSED GREEN on the real gemma-4-12B — §10 — so the forward pass now lives in the same exact-integer container as the memory tier; engine `69c0588` + submodule `d9d96f3`).** On top of the already-closed XBAR stack (Ring-2 verbatim, Ring-3 gist, EAR→Ring-2 bridge — proven end-to-end as of 2026-06-17), the memory tier was moved off the generic float carriers it had been running on and re-carried onto the engine-native dual-prime negacyclic CRT-NTT. The headline finding is a **boundary thesis** (§9): the substrate's value is exact arithmetic — the indestructible algebraic *container* — while every attempt to impose number-theoretic *structure* onto the high-entropy *content* was measured-inert. This is the human-readable map of where the project stands: what we built, the numbers that matter, the tests we used to prove each claim, *why* those tests are the right ones, and why the results can be trusted. It is written to be read top-to-bottom by a person, and to orient an agent in one pass. Detailed, citable records live in the contracts and `PPT-LAT-STATE.md`; this document is the synthesis, not the ledger.
...
### 9.4 NEXT
The host-numpy VSA → native Z_q/NTT port (the long-deferred Ring-3 follow-on) is now **DONE**, and as of 2026-06-18 so is the **native-C `core/`-resident port of the whole Ring-3 / NIGHTSHIFT loop** — `core/ring3/` (`ring3.c` + `include/sp/ring3.h`) ports the VSA layer + the SELECT→BIND→SHADOW-GATE→PROMOTE+EVICT→SEAL state machine onto native `sp_pr_mul`; gate **T_RING3_NATIVE 42/42** (bind/unbind/superpose BIT-IDENTICAL to the Python reference; NIGHTSHIFT reproduced [32,8]@D=1024 + gate-before-cap@D=128; order-immune); carriers unified to splitmix64. Math-core `e0fccd3`, engine submodule `7b992d2`. **So XBAR / NIGHTSHIFT is now COMPLETE end-to-end** — mechanisms + the deployable native-C core. The only remaining XBAR items are optional / deferred-by-choice: the **N1 unattended persistence soak** (deployment endurance, deferrable like the KAIROS ≥24h gate), **G-R3-PROV** (a secondary enhancer), and **Path B** (the learned adapter — only if Path A is measured insufficient).

**On the prior "T4 weights" frontier (corrected):** T4 Frobenius π^k on the *model weights* is **NOT** the next step — it is CONVICTED. The incoherence-rotation / Hadamard-fold compression was ruled REDUNDANT vs the per-32-block OK_Q4B (G-WEIGHT-FOLD-ORACLE `8ae8825`, "do NOT build" — the boundary thesis on weights), and the byte-exact OK_Q8-per-tensor-π^k angle was absorbed into the now-closed byte-exact forward (the dp4a integer accumulate already makes OK_Q4B exact). **NEXT = B4 NIGHTSHIFT (between-turn memory consolidation, pre-scoped) then an open strategic inflection (operator's call):** the major campaigns (XBAR/NIGHTSHIFT mechanisms, byte-exact forward, KAIROS, GNA EAR, and now B3-WC autonomous recall — §11) are all closed; candidate directions are harden→publish (P3.4 larger-N + the R1–R5 prepublication checklist), the gguf-v4 Mersenne co-design, or the diffusion lane. The one carried-forward external item is the 2-physical-GPU byte-exact check.

---

## 10. The byte-exact forward — the FORWARD PASS joins the exact-integer container (2026-06-18)

### 10.1 What happened
The same day the *memory* tier unified onto the exact-integer O_K substrate (§9), the **forward pass itself** was made byte-exact on the real gemma-4-12B. "Byte-exact" here means **exact integer arithmetic → logits bit-identical across reduction-order and machine** — a *cross-machine-determinism / auditability* property, **explicitly not a compression result** (the compression axis is a convicted negative; this is a different axis). The engine ran the *linear* algebra exactly already (the dp4a int4×int8→int32 accumulate is order-immune); what stayed float were four nonlinear "islands" — **RMSNorm, softmax, GELU-tanh, RoPE** — plus the attention Q·K / p·V dot and softmax. All of those now convert to exact-integer behind a default-off `SP_BYTEEXACT` flag, on the **same dual-prime CRT-NTT substrate the XBAR memory ring uses** (frozen primes q1=1073738753, q2=1073732609, Garner inverse 894602413, M=q1·q2≈2⁶⁰ — and because M fits a u64, the whole thing avoids `__int128`: device `__umul64hi` for wide products, a 64-bit-split integer `isqrt` for the RMS normalizer, a device CORDIC for the RoPE angles). One algebra, two jobs: it binds an episode and it computes the forward.

### 10.2 The piece that was already built, and the piece that was new
The honest part of this story is that most of the byte-exact *linear* algebra **already existed** — bounded and bit-exact-gated — in the universal Rust crate `tools/sp_dsp_smoke` (dual-prime Barrett, mod-q matmul, Garner CRT with the very constant `894602413` a CUDA hand-roll had re-derived, and the full NTT ladder). Re-deriving it in fresh prototypes was wasted motion (the lesson is banked: *grep the crate before building byte-exact arithmetic*). The genuinely-new piece was the **four nonlinear islands**, added as `sp_islands_q_ref.rs` (+ math-core `core/exact_islands/`) with **G-ISLANDS-Q-REF GREEN** on the host (RMS 5.8e-6 / softmax 1.3e-6 / GELU 2.8e-6 / RoPE 9.2e-6, all reduction-order-immune; RoPE via fixed-point CORDIC).

### 10.3 The gates — on the real 12B
- **G-BYTEEXACT-ISLANDS-CUDA (GREEN).** Before wiring the integer islands into the CUDA decode, the crate's integer references were checked against the *live* float-island outputs on **real layer-24 12B activations**: RMS 3.84e-5 / GELU 8.18e-7 / RoPE 9.62e-6, all under the 1e-4 contract tolerance. The refs are faithful to the float kernels on real data, so the wire-in will cost nothing measurable. *Why this test first:* it de-risks the expensive kernel conversion by measuring island fidelity on-model, not on synthetic input.
- **G-BYTEEXACT-FORWARD-12B (GREEN) — the keystone.** The integer islands + integer attention now run *inside* the gemma-4-12B CUDA decode, behind `SP_BYTEEXACT`. Three legs: **OFF = PPL 4.6665, byte-identical to the baseline** (the null floor — the one-shot `gemma4_decode_cuda` is left byte-untouched, so every prior closed gate still stands); **ON = PPL 4.6569**, a −0.21% parity move on n=42 (the integer path costs nothing measurable); and **run-to-run bit-identical** with the flag on, which is the cross-machine-determinism proxy (integer reductions are order-immune, so two runs — and, by the same argument, two machines — produce the same bytes).
- **Driven by the universal daemon.** The resident `sp_daemon` drives the 12B *prefill* through the existing forward-backend hook (**G-WIRE-CUDA-GEMMA4**), and now *decodes* it token-by-token through a **new L1 verb** — `sp_session_register_kvdecode_backend` (open / prefill / decode_step / rewind / position / close), the persistent-KV sibling of the prefill-only forward hook (**G-WIRE-CUDA-DECODE-GEMMA4: 32/32 tokens bit-identical to the `gemma4_kv_decode` oracle, VRAM flat / O(1)**). The ABI *grew* (append-only) — it did not break.

### 10.4 Honest scope (kept on the record)
- The **only** remaining item is an **external two-physical-GPU bit-identical check.** On one machine, the run-to-run bit-identity + the integer-reduction order-immunity *are* the determinism proof; a literal two-GPU diff is out-of-band (we don't have a second card on the bench), so it stays an open, named external item — not a claim made and not a claim hidden.
- The ON-leg PPL parity is **n=42, a single chunk.** It is deterministic (not router-sampled, so not a noise-flippable small-N illusion), but a larger-N multi-chunk run is the hardening lever before any public headline leans on the −0.21%.
- **Byte-exact = auditability, not compression.** The integer forward does not make the model smaller or faster; it makes it *exact and reproducible*. That is the entire point — it is the one property a floating-point, FMA-contracting, libm-`expf`/`sinf` stack cannot offer — and it is a *different* axis from the compression levers (which remain convicted, §9.3).

Receipts: engine `tests/fixtures/xbar_r3/{G-ISLANDS-Q-REF, G-BYTEEXACT-ISLANDS-CUDA, G-BYTEEXACT-FORWARD-12B, G-WIRE-CUDA-GEMMA4, G-WIRE-CUDA-DECODE-GEMMA4}.log`; design + gate contract `papers/CONTRACT-BYTEEXACT-forward.md` §5.1/§5.2/§7/§8; ABI verb `papers/PPT-LAT-L1-ABI-v0.md` §6b; loader reconciliation `papers/PPT-LAT-SP-MODEL-v0.md` §6.5. Engine `69c0588`, math-core submodule `d9d96f3`, lattice contract `2751407`. Public: Papers 19/20/21 + `LEDGER.md` in `Position_Is_Arithmetic` (`19f3d8c`).

## 11. The autonomous librarian — B3-WC learned-head recall LIVE on the chat (2026-06-19/20)

The served `/v1/chat` was brought onto the full substrate over the prior session (`CONTRACT-CHAT-FULLSTACK`: coherent + reproducible + byte-exact-by-default + O(1)-context SWA-ring + a single latent entry point, all GREEN — `run_console.bat` → http://127.0.0.1:3000/). This session closed the last open stage: **B3-WC, autonomous learned-head episodic recall.** The operator states a fact, the curator stores it as an episode, and on a later query the model **autonomously selects the right episode — or refuses if none is relevant.**

### 11.1 The arc (a boundary-thesis win, honest negatives intact)
- **Every hand-designed relevance signal FAILS open-world.** 6 verifiers + 4 Stage-2 Disposer signals (Yes/No bridge, Δ-continuation first-token, multi-token ΣΔLL, consensus(q·K ∧ ΔLL)) + cosine-qK normalization + ΔLL-polarity flip — each dominated by per-episode / shared-attractor bias at N=3 wiki (best hand-design = raw q·K + argmax multitok-ΔLL = 2/3 rank; the two "principled" normalizations made it *worse*). Commits `4dba6c8`/`2b623ab`/`acd7b3a`.
- **Root cause (the ablation oracle):** wiki facts are **parametric** — the 12B regenerates them from weights — so ablating the stored copy strands nothing and episodic dependency is *unmeasurable*. The corpus was the wall (B3-v10 `27038d3`).
- **The unlock — teacher-forced ablation knockout on a NOVEL needle.** `SP_B3_SECRET` teacher-forces the known secret tokens and cudaMemset-ablates exactly their source KV rows, scoring ΣΔLL with/without. Novel **−33.56** vs parametric **−0.15** (~220×); B3-v13 3-archetype matrix (code −33.56 / contradiction −18.58 / relational −16.10) vs parametric band [−0.15, +1.45], **TAU pinned −8.0**, ~16-nat gap (`15738c1`/`b6470cc`). This gate is both a *shippable oracle* and a *perfect labeler* (perfect-diagonal separable matrix, `7556d04`).

### 11.2 The corpus + the learned head
A split-entropy curator (`mint_corpus.py` `52ec468`: code=CSPRNG-in-shape so OOD-shock doesn't crater dLL via token-refusal; contradiction/relational=structured-fictional; secret stated ONCE sentence-final) + a single-pass `ep.secret` admission oracle (`adfcb06`) scaled to **200 needles + 60 foreign null-class** (200/200 admitted, control rejected, `f4166c7`) — the ablation-validated ground-truth labels the hand-designed era never had. The learned **W_c** head: HD=512 → r=32 projection; relevance = **logsumexp over positions then mean over heads**; InfoNCE over [episodes + NULL/s0] + a reject-margin hinge.

- **Diversity was the binding constraint at every granularity.** A templated corpus gave 34% instance top-1 (shared carrier sentences collided query-Q intra-archetype); re-minting with unique subjects + varied carriers (`mint_corpus_v2` `72d9c77`, `f62e6ef`) took instance top-1 **34% → 100%** with the SAME machinery.
- **The int16 "degradation" was a wrong-reduction bug, not quantization.** Diagonal under max=12/361, top-8-mean=16/361, but **logsumexp-mean = 361/361 and int16 == f32 for all three.**

### 11.3 The gate and the deploy
- **G-CHAT-B3-WC-DIV2 (`87044d8`, deploy gate):** (E+1)-argmax over [episodes, NULL=s0] = **360/361 instance recall + 50/50 foreign reject, int16-exact, s0=+0.102.**
- **G-CHAT-B3-WC-DEPLOY (`edc8079`, LIVE):** `recall.rs` `WcHead`/`wc_score` (numerically-stable LSE) + `routes.rs` `SP_B3_WC` (score every registry episode → NULL-argmax → replay the winner at `SP_REPLAY_MTARGET=42`, which clamps the injection mass, or keep the clean prompt; default-off = null floor). Live on the 90-needle registry: matched *"Which recovery code authorizes the Marlock mag-rail depot?"* → RECALL `ep_n_div_000` (9.858, clear argmax); foreign *"What is the capital of France?"* → whole population negative → NULL → clean **"Paris."**
- **UX:** `run_console_recall.bat` (`eb6332d`, PMAX 4096 `82d77c8` for the 12 GB card) arms the recall env; the console Send button toggles to a **"■ stop"** (`fc25b07`: client AbortController + a real `/v1/abort/:id` server-cancel route).

### 11.4 What it means
Autonomous recall is won by a **learned head on a diverse corpus** — *not* by hand-designed number-theoretic / geometric signals (every one a measured negative, kept attached). **Corpus diversity** (not machinery, not normalization, not more memory) was the binding constraint at every granularity; **novel-vs-parametric** is the only measurable episodic-dependency axis. This extends the boundary thesis to the recall policy. **B4 NIGHTSHIFT** (between-turn consolidation: turn → curator-mint → ablation-admit → hot-append to the in-memory registry; the W_c head needs no retrain to score a new episode) is DEFERRED and pre-scoped (`SESSION-HANDOFF.md §0d`).

Receipts: engine `tests/fixtures/chat_fullstack/G-CHAT-B3-WC-{DEPLOY,DIV2,DIV}.log`; record `papers/CONTRACT-CHAT-FULLSTACK.md`.
...
| Byte-exact islands (host refs) | **CLOSED GREEN** | G-ISLANDS-Q-REF: RMS 5.8e-6 / softmax 1.3e-6 / GELU 2.8e-6 / RoPE 9.2e-6, order-immune; RoPE via fixed-point CORDIC. Crate `sp_islands_q_ref.rs` |
| Byte-exact islands vs live 12B | **CLOSED GREEN** | G-BYTEEXACT-ISLANDS-CUDA: crate refs vs real layer-24 float islands — RMS 3.84e-5 / GELU 8.18e-7 / RoPE 9.62e-6, all < 1e-4. Engine `b93f157` |
| **Byte-exact forward (12B)** | **CLOSED GREEN** | G-BYTEEXACT-FORWARD-12B: OFF 4.6665 byte-identical null floor / ON 4.6569 parity (−0.21% n=42) / run-to-run BIT-IDENTICAL (cross-machine proxy). `SP_BYTEEXACT`, one-shot decode untouched. Engine `69c0588` |
| Daemon drives 12B (prefill+decode) | **CLOSED GREEN** | G-WIRE-CUDA-GEMMA4 (prefill, `cuda_forward_count 0→1`) + G-WIRE-CUDA-DECODE-GEMMA4 (new L1 kvdecode verb, 32/32 == oracle, VRAM flat O(1)). Submodule `d9d96f3` → engine `6b9a786` |
| 2-physical-GPU byte-exact check | **OPEN (EXTERNAL)** | the ONLY remaining byte-exact item; run-to-run bit-identity + integer-reduction order-immunity are the on-one-machine proxy; literal 2-GPU diff is out-of-band |

**The whole XBAR memory stack is structurally proven end-to-end on the 12B within a 12 GB footprint.** The substrate reads, writes, compresses to O(1), retrieves under poison, replays bit-exactly, and recalls without breaking perplexity (P3). The curator indexes (registry), addresses (256-bit hash), selects (integer Hamming), is inert when off, and on metal promotes the matched recall at zero deflection cost and discards the corrupted one (C2). The neocortical gist tier superimposes many episodes into a bounded vector, recall@1 lossless to N=32, misses gate-caught, idle-loop seals 349.8 MB of resident KV down to a 16.3 KB Ring-3 index (Ring-3 Path A). The EAR→Ring-2 seam is proven for audio episodes, with a foreign-by-design deflection caveat correctly on the record. Endurance: 6h soak GREEN (formal ≥24h gate un-pursued by operator choice, not failed). GNA "EAR" CLOSED on physical silicon (§4.6). **And as of 2026-06-18 the whole memory tier is UNIFIED onto the exact-integer O_K substrate (§9): native CRT-NTT bind (256/256 bit-identical, reduction-order-immune), Frobenius integer Ring-2 store (sub-ULP at 24b), the full organism loop running native on real episodes, and the period-6 rebase CLOSED — bounded by four honest negatives that establish the boundary thesis (the substrate wins on the algebraic container, never on the high-entropy content). **The SAME DAY, the FORWARD PASS itself joined that container: the byte-exact forward is CLOSED GREEN on the real gemma-4-12B (§10)** — the four nonlinear islands (RMSNorm/softmax/GELU/RoPE) + the attention dot/softmax now run exact-integer on the same dual-prime CRT-NTT, OFF = byte-identical null floor (4.6665) / ON = PPL parity (4.6569) / run-to-run bit-identical, driven end-to-end by the universal daemon (prefill + a new persistent-KV L1 decode verb, 32/32 == oracle). Byte-exact = auditability / cross-machine determinism, explicitly NOT compression; the only remaining item is the external 2-physical-GPU check. **And as of 2026-06-20 the served 12B chat does AUTONOMOUS LEARNED-HEAD RECALL (§11): a W_c head selects the right stored episode for a query or refuses (G-CHAT-B3-WC-DEPLOY, engine `edc8079`; 360/361 recall + 50/50 reject offline) — corpus diversity, not machinery, was the binding constraint. NEXT = B4 NIGHTSHIFT (between-turn consolidation, pre-scoped) then the operator's strategic inflection (T4 weights is CONVICTED, §9.4).** Standing hygiene queued: the external 2-GPU byte-exact check, cudaEvent journal-tax, `gemma4_kv_decode` first-token boundary, compact-slab globals wrap-rewind, P3.4 larger-N multi-chunk hardening run.
