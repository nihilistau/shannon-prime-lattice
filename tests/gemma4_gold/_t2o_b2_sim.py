...
_log = open(r"C:\Users\Knack\AppData\Local\Temp\_t2o_b2.log", "a", buffering=1, encoding="utf-8")
...
def q4b(w):                                          # ASYM per-32: q in [0,15], w_hat = q*s+mn
    rows, cols = w.shape
    nb = cols // 32
    blk = w[:, :nb * 32].reshape(rows, nb, 32)
    mx = blk.max(axis=2); mn = blk.min(axis=2)
    s = (mx - mn) / 15.0
    s = s.astype(np.float16).astype(np.float32)
    mn = mn.astype(np.float16).astype(np.float32)
    s[s == 0] = 1.0
    codes = np.clip(np.round((blk - mn[:, :, None]) / s[:, :, None]), 0, 15)
    out = (codes * s[:, :, None] + mn[:, :, None]).reshape(rows, nb * 32)
...
KEEP_Q8 = ("down_proj", "q_proj", "k_proj", "v_proj", "o_proj")
def stq(nm):
    w = st_raw(nm)
...
print(f"B2 (ASYM-32 gate/up, Q8 rest) SIM: PPL = {ppl:.4f} vs gold {ORACLE} -> {rel*100:+.2f}%  [{time.time()-t0:.0f}s]", flush=True)
