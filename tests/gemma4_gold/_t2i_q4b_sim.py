# _t2i_q4b_sim.py — OK_Q4B FORMAT SIMULATION through the gold instrument:
...
_log = open(r"C:\Users\Knack\AppData\Local\Temp\_t2i_q4b.log", "a", buffering=1, encoding="utf-8")
...
def q4b(w):                                          # per-32-block sim, f16 scale discipline
    rows, cols = w.shape
    nb = cols // 32
    blk = w[:, :nb * 32].reshape(rows, nb, 32)
    ...
    out = (codes * s[:, :, None]).reshape(rows, nb * 32)
    if nb * 32 < cols:                               # tail (cols%32) — gemma4 dims are /32 clean
        out = np.concatenate([out, w[:, nb * 32:]], axis=1)
    return torch.from_numpy(out.astype(np.float32))

def stq(nm): return q4b(st_raw(nm))                  # quantized matmul weight
...
def emb_rows(r0, r1):
    ...
    return q4b((raw.astype(np.uint32) << 16).view(np.float32).reshape(r1 - r0, 3840).copy())
erows = torch.cat([emb_rows(t, t + 1) for t in uniq])
x = erows[[rowmap[t] for t in ids]] * (float(E) ** 0.5)
del erows
print(f"q4b sim: {len(uniq)} embed rows gathered", flush=True)
...
print(f"Q4B FORMAT SIM: PPL = {ppl:.4f} vs gold {ORACLE} -> {rel*100:+.2f}%  [{time.time()-t0:.0f}s]", flush=True)
