...
_log = open(r"C:\Users\Knack\AppData\Local\Temp\_t2p_b1.log", "a", buffering=1, encoding="utf-8")
sys.stdout = _log; sys.stderr = _log
import numpy as np
import torch
torch.set_num_threads(6)

SPM = r"D:\F\shannon-prime-repos\models\gemma4-12b-b1.sp-model"
...
def q8(name):                                        # OK_Q8 / OK_Q4B -> torch f32 [rows, cols]
    dt, nd, dims, off, sz = tab[name]
    cols, rows = int(dims[0]), int(dims[1])
    f.seek(data_off + off)
    if dt == 10:                                     # OK_Q8 + .scale
    ...
    if dt == 13:                                     # OK_Q4B + .bscale (per-32 f16)
        nibc = (cols + 1) // 2
        raw = np.frombuffer(f.read(rows * nibc), np.uint8).reshape(rows, nibc)
        lo = (((raw & 0xF) ^ 8).astype(np.int8) - 8)
        hi = (((raw >> 4) ^ 8).astype(np.int8) - 8)
        codes = np.empty((rows, cols), np.int8)
        codes[:, 0::2] = lo[:, :cols - cols // 2]
        codes[:, 1::2] = hi[:, :cols // 2]
        nblk = (cols + 31) // 32
        sdt, snd, sdims, soff, ssz = tab[name + ".bscale"]
        f.seek(data_off + soff)
        bs = np.frombuffer(f.read(rows * nblk * 2), "<f2").astype(np.float32).reshape(rows, nblk)
        w = codes.astype(np.float32) * np.repeat(bs, 32, axis=1)[:, :cols]
        return torch.from_numpy(w)
...
print(f"B1 ARTIFACT GATE: mixed Q4B/Q8 PPL = {ppl:.4f} vs gold {ORACLE} -> {rel*100:+.2f}%  [{time.time()-t0:.0f}s]", flush=True)
