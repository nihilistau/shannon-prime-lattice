...
_log = open(r"C:\Users\Knack\AppData\Local\Temp\_t2n_b1.log", "a", buffering=1, encoding="utf-8")
...
KEEP_Q8 = ("down_proj", "q_proj", "k_proj", "v_proj", "o_proj")  # Q4B only gate/up
def stq(nm):
    w = st_raw(nm)
...
print(f"B1 (Q4B=gate/up only, Q8 rest) SIM: PPL = {ppl:.4f} vs gold {ORACLE} -> {rel*100:+.2f}%  [{time.time()-t0:.0f}s]", flush=True)
