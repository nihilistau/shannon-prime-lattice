...
**Prereqs before any 32k green gate (none done — all harness-only today):** (1) wire the ±1 projection sidecar into the live `qwen3_generate_kv` KV path with a SHORT-ctx parity check first; (2) wire Ring-2 Optane fetch; (3) a long-context eval text/harness. Wire incrementally — the real-model path was fragile this session (the struct-divergence segfault). Bring up at N=2k–8k (cheap, clean) before pushing to the 32k trained edge.
