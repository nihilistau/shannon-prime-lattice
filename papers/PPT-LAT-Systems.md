...
Standard Rotary Positional Embeddings (RoPE) compute relative phase shifts using continuous floating-point trigonometry, leading to immense cache sizes at long contexts. The engine has a path to collapse this via the Three-Gap Theorem (PPT-LAT-Theory T7). **There is a load-bearing precondition that earlier drafts of this section elided:** Three-Gap applies to *linear* irrational sequences $\{k\alpha \bmod 1\}$. Standard RoPE uses a *geometric* schedule $\theta_d = \mathrm{base}^{-2d/D}$ that is **not** three-gap-distributed — so the ~43× cache collapse is **not** a property a stock Gemma3 / Qwen3 / Llama model satisfies out of the box.

The optimisation therefore splits into two separable claims that must be implemented and gated independently:

**1.6.a — Lossless rel-attn cache restructuring (φ-RoPE conditional).** When the model's RoPE frequencies are linear multiples of $\varphi$ (i.e. the model was pretrained or fine-tuned with a φ-derived schedule), the phase shifts $\Delta \cdot \theta_d$ for fixed relative offset $\Delta$, when sorted by frequency, exhibit at most three distinct discrete increments by Theorem T7. The relative-attention cache then collapses from $O(\mathrm{ctx} \cdot D)$ to $O(\mathrm{ctx} \cdot 3)$ entries — for $D=128$, $\mathrm{ctx}=4096$ that is $12{,}288$ entries vs $524{,}288$, the ~43× figure. The dimension-ordering sort key is identical to the one VHT2 + Möbius uses to surface the squarefree anchors, so no second sort is required. **On a stock geometric-RoPE model this gate is a no-op**: the precondition fails, the cache restructuring is not applicable, and the engine takes the standard path. Gate: `SP_ROPE_3STEP_CACHE=1`; activates only when the loader reports `rope.style=phi`. Acceptance: argmax + KL parity within E_B_2 tolerance, conditional on a φ-RoPE model being loaded.

**1.6.b — RoPE frequency-schedule swap (quality-trading research path).** Replaces the model's geometric schedule with $\theta_d = \{d\varphi\} \cdot 2\pi$. This is a genuine positional-basis change: the model was trained with one schedule and is being asked to infer with another. On stock pretrained models this degrades quality at long context until the model is fine-tuned on the new schedule. Validation is therefore a PPL-drift sweep on a calibration corpus (mirroring the Phase 4 Fibonacci-KV item), not argmax/KL parity. Gate: `SP_ROPE_PHI=1`; default OFF; intended as research infrastructure for E9.1-style φ-RoPE pretrained or fine-tuned models, **not** as a production switch on stock weights.

When both gates are on (`SP_ROPE_PHI=1` followed by `SP_ROPE_3STEP_CACHE=1`), 1.6.a's precondition is satisfied and the cache collapse becomes lossless — but downstream quality has already been traded by the schedule swap. Stacking the two is the path for models that are intentionally trained or fine-tuned with φ-RoPE; until such a model exists in production, 1.6.a alone is a no-op and 1.6.b alone is a quality-trading swap that needs PPL-drift acceptance.

- **Backend impact (1.6.a + 1.6.b together).** All four backends benefit; the win is largest on CPU (where trig calls dominate) and Hexagon (where transcendentals are expensive).
- **Regression invariant.** With both gates off, the engine produces bit-identical output to the standard RoPE path. With only 1.6.a on, the gate auto-no-ops on stock models and the output is still bit-identical. With 1.6.b on, the output changes by design.
