...
**Status:** SPEC (design lane, banked; not yet gated). **Parent:** RFC-XBAR v1.1 §Ring-3 amendment (the consolidated "neocortical/gist" tier, dual-recall, `G-R3-LOSS`); CONTRACT-XBAR-C1-lite (the curator); `DESIGN-diffusion-lane.md` (the consolidation-time gist instrument + the §4 recall-time-upsampling trap); CONTRACT-XBAR-P2b §3n/§3o (why VSA is NOT the curator).
...
**Keys are ENGINEERED near-orthogonal carriers, not raw activations.** `key_i ∈ Z_q^d` is a deterministic address — either a frozen ±1 Rademacher draw (the `sp_arm_build_R` seed discipline) or a **Zeta-PE prime-harmonic** carrier (the operator's `prime_encoding` research), chosen *because* it is near-orthogonal and well-conditioned in the NTT domain. The **value** `value_i` is the episode gist descriptor (the curator's k=2 pseudo-tokens, or a pooled episode vector) — STORED, never injected raw.
...
- **Carrier conditioning.** Whether prime-harmonic (Zeta-PE) carriers are near-orthogonal *enough* in `Z_q` vs a plain Rademacher draw is an empirical question (a cheap one — measure pairwise NTT-domain coherence first).
