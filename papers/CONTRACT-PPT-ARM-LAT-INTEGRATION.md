...
- **G-INT-1 — Spinor→Ring-2 persistence (the first wire).** A NIGHTSHIFT-captured episode is serialized as VHT2 63-byte spinor blocks (0xA5 @ byte 63), written to the Ring-2 Optane backend, indexed by its 256-bit C2 sig, and reloaded. PASS iff: spinor round-trip is **byte-identical** (capture D2H == spinor-encode/decode == Optane write/read == reload), AND the reloaded episode's `wc_score`/sig match the in-mem capture exactly. Default-off (no `SP_RING2_SPINOR`) = current `ep.k`-file path, byte-identical.
