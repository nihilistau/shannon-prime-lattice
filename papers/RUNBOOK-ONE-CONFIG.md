...
| **`run_console_reason.bat`** (rewritten 2026-07-10, AUDIT) | REASON model + full memory stack: Tier 0 (PMAX=12096) + Tier 1 (production registry, `SP_AUTO_RECALL_DEFAULT=1`, `plain`) + Tier 2 growth (`SP_B4_NIGHTSHIFT`/`SP_NIGHTSHIFT_PERSIST`/`SP_MEM_STORE`) + `SP_QKEY_MINT=1` + `SP_MEM_CLASSIFY=1` + `SP_MEM_POLICY=1` + `SP_SPINE=1` + veto head | **FIXED — was memory-broken**: prior revision had `set "SP_NIGHTSHIFT_PERSIST"` (no `=1` — a batch no-op), no `SP_RECALL_REGISTRY`, `SP_RECALL_L5=0` → episodes orphaned, nothing persisted across sessions. Auto-recall flipped to 1 because question-space keys (`SP_QKEY_MINT`) are the landed condition quiet-memory mode was waiting on. |
| `run_console_everything_reason.bat` (updated 2026-07-10) | as `run_console_everything.bat` on the REASON model, + `SP_QKEY_MINT=1`/`SP_MEM_CLASSIFY=1`/`SP_MEM_POLICY=1`; quiet-memory default kept (`SP_AUTO_RECALL_DEFAULT=0`, console checkbox overrides) | LIVE-PLAY |
| `tools\memory_doctor.py` (new 2026-07-10) | registry hygiene: `audit` / `prune-dead` / `adopt` orphans / `remint` via the store verb | maintenance tool — the daemon silently skips registry rows whose episode dirs were deleted and never rediscovers orphaned `_nightshift_live` dirs; this makes both visible/fixable |
...
| `run_gateway.bat` / `run_agency.bat` | harness on top of a running daemon | superseded for daily use by `run_gateway_system.bat` (run_gateway.bat arms NOTHING and points memory at the stale `_seed_corpus`) |
| **`run_gateway_system.bat`** (new 2026-07-10, AUDIT) | armed agent gateway :8800 — `SP_SPINE_TOOLSET=1` + `SP_SPINE_RECALL=1` + `SP_PERSONALITY=1` + `SP_MCP_TOOLS=1`, memory on the production `_memory_live` registry, `SP_DAEMON_URL=:3000` | THE agent gateway. Without it there is NO tool calling / tool cards / persona editing anywhere — the daemon serve is agent-blind by design. Console autodetects it (index.html probes `:8800/health`). |
| **`run_console_agent.bat`** (new 2026-07-10, AUDIT) | one-shot full stack: `run_console_reason.bat` (daemon, waits 25s) then `run_gateway_system.bat` | the daily-driver ENTRY POINT for the full agentic console |
| `harness python -m harness.mcp_server` (new 2026-07-10) | FastMCP server: system tools (fs/shell/powershell/python/web/time/memory) over MCP; `mcp_servers.json` + `SP_MCP_TOOLS=1` bridges external MCP tools INTO the model's tool loop | gate `tests/h_mcp_server.py` G-MCP-SERVER 3/3; see harness MCP-README.md |
