...
- **▶▶▶ THE FORWARD PLAN — TWO STRIKES, in priority order (2026-06-21).** The recall organism is unified + recorded in [`RFC-ORGANISM-unified.md`](RFC-ORGANISM-unified.md) (served byte-exact O(1) 12B chat + live latent W_c RECALL selector + causal-ablation ADMISSION oracle + NIGHTSHIFT offline curator + MEM-OKF store + PoUW receipts). Honest tiers: the curator is **gated-GREEN-on-synthetic** (criterion-5 live PENDING); the diffusion judge is **UNPROVEN / in-drawer**.
  ...
  - **Strike 2 — the OOD diffusion kill-test (settle Phase 5).** Mint a held-out OOD novel corpus W_c never trained on; run the fast `llama-diffusion-gemma-eval` oracle vs the live W_c head on zero-shot recall + foreign-reject. **If the diffusion oracle does NOT decisively beat W_c, the Phase-5 native diffusion judge lane (incl. N5b resident reservoir) is RETIRED on the record.** W_c already beats the diffusion oracle IN-distribution (360/361 vs 95.6% on `_needle_corpus_div`); the **only honest battlefield is OOD**. The 95.6% is the EXTERNAL llama.cpp PR-24423 oracle's number (Q4_K_M), NOT our engine — our native single-forward judge was FALSIFIED ~25% (`f8f76a5`). N5b ([`DESIGN-diffgemma-n5b-reservoir.md`](DESIGN-diffgemma-n5b-reservoir.md)) stays gated on the judge winning. Record `CONTRACT-NIGHTSHIFT-CURATOR.md` §4 Track 2 + `RFC-ORGANISM-unified.md` §5.
