...
- **>>> TODAY'S EDGE (2026-06-24) -- diffusion-judge perf SHIPPED (~2x, byte-exact) + a recall regression to chase + the Cola north-star.** (a) `SP_DG_SCRATCHREUSE` default-on ~1.46x (engine `e31c70d`) + `SP_DG_ASYNC` byte-exact pinned double-buffer prefetch of spillover experts, ~2x stacked (engine `2a1c830`, default-OFF); concurrency stress 1440/1440 MOECHK byte-exact OFF==ON. (b) **IMMEDIATE NEXT -- a real regression to diagnose:** the `dg_self_cond` OOB fix (harness CL=16 vs forward C=256 -- a V=262144 vocab-softmax over-read) that unblocked async parity ALSO exposed a judge-quality regression: full-corpus recall on the fixed baseline is severely degraded (single-digit % vs the prior 95.6% oracle), so the buggy self-cond was *load-bearing* for the judge's step-0 discrimination. ADJUDICATE: is step-0 self-cond supposed to be zero, or does the judge need the full multi-step entropy-bound denoise (N3/N4 sampler depth -- the 95.6% ran 48 steps)? HOLD `SP_DG_ASYNC` default-on until resolved (async is byte-exact vs baseline either way -- a judge-quality issue, not concurrency). (c) **COLA block-causal NORTH-STAR** ([`DESIGN-COLA-DLM-MAPPING.md`](DESIGN-COLA-DLM-MAPPING.md)): block-causal attention EXPLAINS the prefix-KV refutation (bidirectional => prompt K/V not canvas-invariant); it is a TRAIN-TIME property, NOT an inference mask flip. E1 read the reference diffusion mask (FREE -- also settles the prefix-KV retire/keep fork) -> E2 inference block-causal probe (expected-degrade) -> E3 O_K-latent diffusion feasibility -> E4 block-causal finetune (deferred, the real prefix-KV unlock). Detail: `DESIGN-COLA-DLM-MAPPING.md` + memory `project_perf_wholemachine.md` / `project_n6_prefixkv.md`.
