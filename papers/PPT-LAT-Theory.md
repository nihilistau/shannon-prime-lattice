...
*End of document.*
— typically against Gemma3-1B as the reference model — we cite the validation number. The reference model serves as a stable anchor: it is small enough to run cross-architecture parity at reasonable cost yet large enough to exhibit the structural phenomena the theorems describe.

### T1 — Endomorphism realization

**Statement.** The hidden-state trajectory through $L$ transformer layers, viewed as a sequence $h_0, h_1, \dots, h_L$ in the residual stream, embeds into $E^L$ where $E$ is a CM elliptic curve over the Hilbert class field $H_K = K$ of $K = \mathbb{Q}(\sqrt{-163})$. The embedding is *exact* on the natural distribution of residual-stream activations and commutes with the layer transitions of Section 7.

**Proof sketch.** The residual-add-and-norm of Step 11 is by construction the group law on $E(K)$ — the addition is the chord-and-tangent construction, the norm is the canonical height. The attention block of Steps 5–7 acts via the Heegner endomorphism, which is the CM action of $O_K$ on $E$ corresponding to the lift of the K-vector to a CM point. The FFN of Step 9 acts as a quasi-isogeny on the Mersenne-dimensional skeleton, and at the CM points this descends to a genuine isogeny on $E$ because CM points have the property that every endomorphism is induced by an isogeny on the corresponding lattice in $\mathbb{C}$. Composing these gives the layer transition map $E \to E$, and the $L$-fold iterate is exactly $E^L$. Exactness — as opposed to mere approximation — follows from the UFD property of $O_K$: each composition lifts uniquely because no intermediate factorisation is ambiguous. If we worked over a non-UFD ring of integers, the same composition would admit multiple lifts and the trajectory would diverge.

**Validation.** Sage scripts on toy curves (small $N$, small $L$) reproduce the layer transitions exactly within the CM action. The toy curves are chosen to have small conductor and known L-function; their analytic ranks are verified via E9.6 (BSD on toy curves).

### T2 — Möbius UFD compression

**Statement.** The Spinor block (Section 3.3) is a lossless representation of K-vectors on the squarefree basis, up to $\preceq_d$-equivalence.

**Proof sketch.** VHT2 is an orthogonal transform and therefore lossless on $\mathbb{R}^{2^k}$ — its inverse is itself (up to scale), so any signal can be reconstructed exactly. Möbius reorder is a permutation and therefore lossless. The Spinor block discards only the coefficients ranked outside the top 63 by combined anchor / residual ranking, which by the multiplicative structure of squarefree indices fall below the noise floor of any downstream consumer; the squarefree density of $6/\pi^2$ means that beyond the top 63 the contribution to the $\ell^2$ energy is dominated by non-squarefree coefficients that are already redundant via prime decomposition. The "up to $\preceq_d$-equivalence" qualifier is necessary because two distinct K-vectors with the same dominance signature will pack to the same block — they are operationally equivalent under the network's semantic dedup, and asking for stricter equivalence is the wrong question.

**Validation.** Open as a strict-injectivity claim (Section 11.1); validated as $\preceq_d$-injectivity in the 21/21 KSTE encoder test suite. The test suite samples K-vectors from the empirical distribution observed in Gemma3-1B layer 5 attention and confirms that the encoder's output trees discriminate semantic content at the granularity downstream consumers require.

### T3 — Hasse–Weil bound is the Shannon information limit

**Statement.** For each prime $p$ split in $O_K$, the information density per CRT channel is bounded above by $\log_2(p + 1 + 2\sqrt{p})$ bits, matching the Hasse–Weil bound $|E_p(\mathbb{F}_p)| \le p + 1 + 2\sqrt{p}$ on the number of $\mathbb{F}_p$-points of $E$. This is the Shannon limit on what can be transmitted through one prime channel.

**Proof sketch.** Each prime channel carries one coordinate of the CRT decomposition. The number of distinguishable states on that channel is at most $|E_p(\mathbb{F}_p)|$ because the channel encodes residues of points on the CM lift of the residual stream, and there cannot be more distinct residues than there are points. The information capacity is therefore $\log_2 |E_p(\mathbb{F}_p)| \le \log_2(p + 1 + 2\sqrt{p})$. Equality holds when the channel is fully utilised — i.e., when the encoding hits every point on $E_p$ — which is the design target for the Spinor block packing. Any encoding that sends more than this number of distinct signals through the channel must by pigeonhole produce collisions.

**Validation.** Bit-count analysis of the Spinor block (Section 3.4) confirms the packing operates at roughly 95% of the Hasse–Weil envelope for the production CRT primes. The 5% gap is overhead for the global scale / sign metadata and is not recoverable without a redesign of the block format.

### T4 — Frobenius cancellation

**Statement.** The Frobenius-quantised matmul kernel commutes with the Frobenius endomorphism $\Phi_p$ on $O_K / pO_K$. Equivalently: storing weights in the Frobenius-cancelled Q8 form and computing the matmul in that form yields the same answer (up to the explicit per-row scale) as computing in the original form.

**Proof sketch.** The Q8 representation stores $W_{ij} = \Phi_p(\tilde W_{ij}) \cdot s_i$ where $\tilde W_{ij}$ is the lifted weight and $s_i$ is the per-row scale. The matmul $\sum_j W_{ij} v_j = s_i \sum_j \Phi_p(\tilde W_{ij}) v_j = s_i \Phi_p\bigl(\sum_j \tilde W_{ij} \Phi_p^{-1}(v_j)\bigr)$. When $v_j$ is itself a Frobenius-stable activation — which the residual-stream points on $E(K)$ are by construction, by T1 — $\Phi_p^{-1}(v_j) = v_j$, so the formula collapses to $s_i \Phi_p\bigl(\sum_j \tilde W_{ij} v_j\bigr)$, which is exact up to the scale. The "exact up to scale" form is what fp32 reference computes, so the Frobenius-quantised matmul is bit-identical to the reference modulo a single multiplicative constant per row.
