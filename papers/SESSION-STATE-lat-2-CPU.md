...
- **Piece 3 — composability + format-lock.** **`COMPOSE`** (`tests/test_compose.c`): with the arena active (`SP_ARENA=q8`), block KV still == f32 KV ⇒ weight source and KV layout are orthogonal, all three gates compose. **Format-lock** = file-scope `_Static_assert`s: `arena.c` (`SP_ARENA_LAYOUT_VERSION==1`, Q8 qmax 127 / Q4 qmax 7, 4-byte f32 scale), `forward.c` (63-byte block, 55-anchor body, `SP_SPINOR_LAYOUT_VERSION==1`). Silent layout drift is now a compile error.

**Load-bearing plan (remaining, dependency-ordered; CUDA after):**

1. **T_FRO_4 (next)** — Gemma3-1B PPL within 0.1%. **Path correction:** the model is the single file `d:\Files\models\Mine\gemma-3-1b-it\gemma-3-1b-it-f16.gguf` (not the `…-f16\` subdir the docs referenced); update the CMake cache var when wiring it. The arena is the production layout this validates. Needs the **2nd architecture** (Gemma3: norms/attn/softcap) + **SentencePiece** tokenizer (not GPT2-BPE) + a PPL loop. Substantial; its own session.
2. **2-CU** (§8.3) — now unblocked (load-bearing layout solid). Host reality: **CUDA 13.2 + RTX 2060 sm_75** (not the old 12.4 pin); `env-cuda.bat`→13.2 (+ likely the ASCII/goto fix). The arena (`SP_ARENA_LAYOUT_VERSION=1`, per-row Frobenius) and the Spinor KV block (63 B, `SP_SPINOR_LAYOUT_VERSION=1`) are the frozen byte layouts the CUDA kernels must read.
