...
> **NEXT SESSION PICKS UP HERE → Gemma3-1B SP1 COMPLETE (forward green); next is SP2 (SentencePiece tokenizer) + SP3 (PPL loop) to close T_FRO_4, then 2-CU.** SP1 (Gemma3-1B CPU forward pass) is **DONE 2026-05-22**: the engine's Gemma3 f32 forward distributionally matches the clean-llama.cpp oracle — **M_GEMMA3_CPU green** (argmax 6/6, top5 6/6, mean KL 1.65e-6 < 1e-5 gate, on "The capital of France is"), full CPU regression **18/18**. Commits (engine, **local — push pending user auth, classifier blocks direct main push**): Task 1 `8aba0e2` (kernels.{h,c} extract), Task 2 `1626f3c` (loader + GEMMA3_BIND), Task 4a `62142c2` (`kernels_attn_head` windowed GQA), Task 4b `0aedcb9` (`gemma3_forward`), Task 5 `8cacfce` (M_GEMMA3_CPU + dump_layers gemma3 checkpoints). **All arch constants confirmed directly from the oracle source** (`gemma3.cpp` + `llama-hparams.{h,cpp}`): plain RMSNorm (the `(1+w)` is baked into the GGUF weights — do NOT add 1), local rope 10000 / global 1e6, global layers `il%6==5` (il=5,11,17,23), query scale 1/√256, GELU-tanh, embed ×√1152, sandwich norms (residual after post-norm), tied head, no softcap. `gemma3_ref.bin` lives at the model dir (`SP_GEMMA3_REF`, not committed). **T_FRO_4 remaining = SP2 (SentencePiece/`tokenizer.model`) + SP3 (PPL loop within 0.1%)** — the M-test uses raw oracle token IDs, no engine tokenizer yet.
...
**Load-bearing plan (remaining, dependency-ordered; CUDA after):**

1. **T_FRO_4** — Gemma3-1B PPL within 0.1%. **Model path:** `D:\Files\Models\Mine\gemma-3-1b-it\gemma-3-1b-it-f16\gemma-3-1b-it-f16.gguf` — `gemma-3-1b-it-f16` is a *subdir* holding the f16 GGUF (the original `…-f16\` doc path was right; an earlier note wrongly "corrected" it to a single file — reverted). Quantized variants (Q3_K_M / Q4_0 / Q4_1 / Q4_K_M / Q6_K / Q8_0 / QAT-Q4) are sibling subdirs; the **SentencePiece** `tokenizer.model` + `tokenizer.json`/configs live at the `gemma-3-1b-it\` root. The arena is the production layout this validates. Needs the **2nd architecture** (Gemma3: norms/attn/softcap) + SentencePiece tokenizer (not GPT2-BPE) + a PPL loop. Substantial; its own session.
