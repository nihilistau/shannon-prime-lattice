...
- **BUILD (the recurring root cause, diagnosed 2026-06-02).** The engine is GCC-authored and was **never made MSVC-clean**; the **MinGW `build` dir SEGFAULTS at runtime** (`test_gen_kv` 0xC0000005, known-good code). So the CPU build does not cleanly build+run on EITHER toolchain. **Env pin FIXED + committed** (engine `33c6a27`): `scripts/env/env-common.bat` now pins `SP_PIN_VS_BUILDTOOLS=D:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools` (MSVC v14.50, cl 19.50) — the prior `...\2019\BuildTools` pin was the phantom P0.1 flagged. With VS18 the build now finds the SDK + compiles 40+ objects (was "can't find stdint.h"). **Remaining MSVC blockers to make VS18 build clean (NOT yet fixed):** (1) per-function `__attribute__((target("...")))` in 5 AVX512 TUs (ifma, persist, spinor, ternlog, vnni) — MSVC rejects; needs an `SP_TARGET()` macro (GCC attr / empty on MSVC + rely on `/arch:AVX512`); (2) `__attribute__((aligned(64)))` on arrays in ternlog — port to C11 `alignas`; (3) `<stdatomic.h>` in `core/sp_channel/sp_hedge.c` — add `/experimental:c11atomics` to MSVC C flags. (`avx512_persist.c` atomics already ported, `33c6a27`.) **ALTERNATIVE path:** debug the MinGW runtime segfault (native toolchain, zero porting) — a clean MinGW rebuild may fix it (the `build` dir may be stale). **DECISION NEEDED:** finish the MSVC de-GCC migration vs fix the MinGW segfault. Until one lands, the SP-side tok/s (SPEED_BASELINE) cannot be measured (the `sp_toks` harness is committed + builds, but no runnable binary yet).
