...
## 5.14 XBAR — THE AUDITABLE LATENT CROSSBAR (RFC v1.1; P1 CITABLE, P2 Phase-0 closed, C1-lite live) (2026-06-09)
...
- **NEXT (decode surgery, fresh session): C1L.0b** — non-truncating `sp_arm_ring2_stdio_open` replay path + wire loaded episode/`projk` into a `qwen3_ppl_decode` replay mode + **G-C1L-0b** null (replayed un-curated episode reproduces the inline run's same-window PPL within the Spinor bound). `core/forward/decode.c` is UNTOUCHED by all XBAR work (last touch `9cd502f`) — C1L.0b opens on a known-good baseline. Then C1L.2 cold-evict, P2.b training (cloud), P3 ring-on-gemma4-CUDA, XBAR-C/M/N.
