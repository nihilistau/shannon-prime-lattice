...
**Tier: BUILT + gated-GREEN (criteria 1-4 synthetic), default-off (`SP_NIGHTSHIFT_OFFLINE`); criterion-5 (B4 distributional/provenance fix) CLOSED live (commit `3ccba61`; receipt `G-CHAT-B4-NIGHTSHIFT-provenance.log`).**
...
The documented **B4 distributional-shape mismatch** is now **FIXED** (commit `3ccba61`, receipt `G-CHAT-B4-NIGHTSHIFT-provenance.log`): the root cause was a 2-token tokenization mismatch (the live path dropped the forced BOS + trailing-newline token that the curator keeps → a different `ep.k`), plus a process-static write-once guard that silenced every live capture after the first in the long-lived daemon. Capturing through the *same* batched `gemma4_decode_cuda` the curator uses + byte-matching the tokenization makes the live episode byte-compatible with the curated `ep.k`, so the deployed head scores it with ZERO retraining: the recorded **live-0.084-vs-curated-9.858 gap is closed** (identical text → 9.858==9.858; novel → in-band + foreign-reject). The remaining novel-instance out-ranking is a general-relevance-head matter, now on the L5-cosine hot path.
...
- **Strike 1 (NIGHTSHIFT criterion-5) — CLOSED (provenance/distributional fix, commit `3ccba61`).** Live-captured episodes land in-distribution: identical text scores live==curated (9.858, the 0.084 collapse gone), a novel fact scores in-band (6.295) + clean foreign-reject (−15.498). Machinery end-to-end correct. Residual (pre-scoped): novel-instance out-ranking under the *closed-set W_c head* — now superseded by the general **L5-cosine** hot-path selector; follow-on = gate L5 recall on a nightshift-captured episode.
