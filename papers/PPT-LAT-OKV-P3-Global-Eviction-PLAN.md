...
## 3. Build steps (P3.x)
1. **P3.0 — verify the wiring point. ✅ DONE (audit, 2026-06-30).** The ARM slab lives entirely inside **`gemma4_decode_cuda`** (`cuda_forward.cu` ~2330–3828, the Beta standalone/test decode loop). It is **absent from `g4_kv_step`** (line 4090, the daemon's persistent decode via `gemma4_kv_open/prefill/decode_logits`) — `g4_kv_step`'s global branch (4181–4206 store, 4207+ attention) does a plain full-cache `dKc[L]` write + `k_attn_decode`, no slab. So the port is real, and it is NOT a clean lift — the slab depends on a wide state web all set up in `gemma4_decode_cuda`'s preamble: the host-RAM store (`arm_ramK/V`), the LSH router (`SP_ARM_LSH_R` → `arm_dR`/`k_proj_RT` → the `arm_pk` r-dim sidecar of projected keys), the per-step scoring (`k_qk_scores` on the sidecar → `arm_dscores`), **host** top-B selection (`sp_oracle_top_b` / `sp_arm_select_geom`), the union build (`arm_slotmap`/`arm_union`/`arm_seen`, sinks∪pos∪top-B capped at `arm_Bslab`), contiguous staging (`arm_stageK/V` → one H2D), abs→compact-slot index remap, and the `k_attn_decode_gather` kernel. **Scope reality:** ~300 lines of intricate CUDA + that state must move into `sp_g4_kv` (alloc at `gemma4_kv_open`, free at close), driven per single-query step. It touches the byte-exact + persist hot path. **Gate blocker (honest):** the win (retrieval past the slab) requires a long-context NIAH, which on this 12GB card thrashes near-OOM (measured this session) — so P3 cannot be cleanly *gated* here even once written. Conclusion: P3 is a real multi-step CUDA port that wants (a) a dedicated session and (b) a gate path with VRAM headroom (a bigger card, or the byte-exact-when-resident gate at small ctx as the only locally-runnable check). This is recorded so the next session starts at P3.2 with the map in hand, not a re-audit.
   - *Original P3.0 text (kept):* Confirm whether `gemma4_kv_step` already branches on `SP_ARM_SLAB/SP_ARM_LSH` (it does not), or whether the slab path is only reachable from the test bin (it is).
