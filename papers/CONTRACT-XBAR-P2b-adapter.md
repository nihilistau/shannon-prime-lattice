...
**Layer-tap sweep + the leakage guard (the honest trap).** Which `L` matters: too early → context not yet integrated (≈ raw embeddings, no lift); too late → the state has rotated toward next-token prediction and begins to **leak the continuation**, so recovery could rise for a *cheating* reason (the input already encodes the answer) rather than genuine compression. Sweep `L ∈ {NL//4, NL//2, 3·NL//4}` and guard with: **(1)** the **readback invariant** (regenerate the span's OWN tokens through the k=2 bottleneck) is the primary metric — it tests span reconstruction, not continuation-copying, so a readback lift is real; **(2)** prefer the **earliest** `L` that lifts (minimal integration that resolves the ambiguity) and report the **full sweep**, never the best single point. If only a terminal `L` lifts while readback stays flat, that is leakage — declare it a null, not a win.

**Gate G-P2b-CTXSTATE + FALSIFICATION (pre-stated).** At the pinned point, matched corpus/seed/horizon, contextualized-state vs raw-embedding input across the L-sweep: **LIFT** — recovery_med clears the context-free plateau (~0.18–0.22) toward the 0.94 ceiling AND the readback recall floor holds (≥75) AND the lift appears at a **mid** (not only terminal) layer ⇒ the ceiling was **information**, and Fork-3's learned cross-attention is likely unnecessary (the frozen model already did the work). **KILL** — no `L` lifts above the plateau with readback held ⇒ input-side context integration does NOT break the ceiling ⇒ the **k=2 steering channel through the frozen model is the true physical limit**; the lever then moves to **k / the injection mechanism / the loss**, not more context, and both Fork-4 and Fork-3 are retired as information plays. SCOPE CAVEAT (same as §3n): a state-derived gist is encode-context-specific; encode-ctx ≈ recall-ctx for episodic replay (the right lever for the *measured* gap), cross-context portability is a logged downstream item.
