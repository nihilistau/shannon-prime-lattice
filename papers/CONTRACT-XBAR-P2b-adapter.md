...
**Status:** SPEC. Build the byte-inert prefix-KV path in the gold forward + the curator KV head + toy-smoke — all LOCAL (no cloud in the design/smoke loop). First probe may inject at a layer subset before scaling to all-layer (cheaper to validate the mechanism). LAUNCH (real-data run) sequenced behind a working toy-smoke; lane = RunPod on a refresh (the lane that completes these), NOT Colab (the §-colab keep-alive/preemption failures, 2026-06-12).
