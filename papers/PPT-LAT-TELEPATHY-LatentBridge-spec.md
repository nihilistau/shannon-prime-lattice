...
| `G-TELEPATHY-TWOSTAGE` | the **two-stage primitive** is cemented: `decide_route`(latent) → `delegate_execute`(CLEAN TEXT); the never-fuse contract is enforced; null floor on LOCAL | `sp-daemon SP_TELEPATHY=1` (`telepathy.rs`) | **GREEN** (route→Telepathy → delegate_execute invoked w/ clean-text task; native L1 coder-exec = scoped wiring) |
