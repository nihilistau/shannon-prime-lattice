...
- **Routing (decided):** Telepathy gets its **own primitive**, NOT an overload of the Tool/Action heads (those decide local effects; transport is orthogonal and must carry *which bridge*). The Route head reuses the head probe machinery and must pass near-miss hardening (TS-2/TS-3 discipline) before it may fire; until then `decide_route → Local`.
- **Next:** the **cross-family destination FORWARD** — wire a minimal Qwen forward + the embedding-prefix inject so a live Gemma→Qwen transmit runs end-to-end in the daemon (the one true-live gap); train+harden the **Route head**; harden REJECT with in-domain-but-wrong negatives; reverse direction (qwen→gemma); realize the license/attestation enforcement (currently SPEC).
