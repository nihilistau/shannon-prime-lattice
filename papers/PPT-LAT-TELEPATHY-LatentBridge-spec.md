...
- **Next (build):** **v1 — the Qwen sidecar** (close the live gap end-to-end with no Gemma-engine change); then harden REJECT with in-domain-but-wrong negatives; reverse direction (qwen→gemma); realize the license/attestation enforcement (currently SPEC).
