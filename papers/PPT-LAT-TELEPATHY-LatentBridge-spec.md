...
- **Next:** wire the bridge into the engine (`SP_TELEPATHY` default-off `LatentBridge` in `eagle_accept.rs`); harden REJECT with in-domain-but-wrong negatives; reverse direction (qwen→gemma); the license/attestation enforcement (currently SPEC).
