...
- **Next:** **multi-vector injection-steering** — inject the mapped per-token *sequence* into Qwen (per-position, late residual) and show it transmits more than the single pooled vector (a reconstruction/readout bandwidth gate); harden REJECT with in-domain-but-wrong negatives; reverse direction (qwen→gemma).
