...
| L5-direct recall (`SP_RECALL_L5`, τ=0.30) | **REGRESSED 2026-07-02** (was GREEN-LIVE) | `d9099cd` / repro `ad27b1b` | `G-L5-RECALL-LIVE` + `G-L5-OBEY-REPRO` | **DOWNGRADED — the 86.89% paraphrase-obey receipt is NOT reproducible on the current stack** (receipt-exact serve+harness: 40.98%; exact `a14fee4` src+lock rebuild: same leak; scaled prompt 63.93% vs its 85.2% receipt). SELECTION intact (54/61 = 88.5% == offline); EXACT-query obey, SNE zero-inference decline, foreign-reject, within-run determinism all still GREEN. Fragile surface = float prefill/logit path feeding the thin obey margin. Repro matrix + pre-scoped root-cause: engine `tests/fixtures/chat_fullstack/G-L5-OBEY-REPRO.log` |
