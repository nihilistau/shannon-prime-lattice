...
- **SP-SWARM — the portable memory profile made distributed [PROVEN-LIVE, default-off].** §6's "portable memory profiles" + the Fibonacci-Prime DHT idea (§ φ tooling) are realized as a *private, invite-only* replication + discovery mesh over the tiered MEM-OKF store: the same content addressing that makes memory portable also makes it **content-addressed replicable** across the operator's own nodes. Five gated layers, each reusing a proven asset — L1 addressing (`sha256`/C2 classes), L2 have/want replication with verify-on-arrival, L3 Ed25519 sign/verify-vs-roster provenance, L0 QUIC (quinn/rustls, reused from the engine's own transport) + Ed25519 mutual roster handshake, L4 C2-SimHash discovery gossip. The `sp_swarm` Rust crate is byte-parity-proven against the Python prototype and integrated into `sp-daemon` behind an optional, default-off `swarm` feature. It rides the byte-exact content addressing + the receipt ledger — it does NOT ship raw latent (ADR-002: the mesh moves curated records, not geometry). Gates `G-SWARM-*` (9); spec [PPT-LAT-DESIGN-SWARM-MEMORY-MESH.md](PPT-LAT-DESIGN-SWARM-MEMORY-MESH.md); call surface [PPT-LAT-MESH-API.md](PPT-LAT-MESH-API.md).
