...
**Last rewrite:** 2026-05-21 · **Last amended:** 2026-06-24 (diffusion-judge ~2x perf SHIPPED [scratch-reuse default-on + async byte-exact, engine `e31c70d`/`2a1c830`]; a self-cond-OOB recall regression IN FLIGHT [HOLD async default-on]; + the Cola block-causal latent-diffusion north-star doc -- see the top forward-edge box. Prior 2026-06-21 (**NIGHTSHIFT OFFLINE CURATOR GREEN-on-synthetic + MEM-OKF anti-rebuild store ACTIVE; the recall organism is unified + recorded in [`RFC-ORGANISM-unified.md`](RFC-ORGANISM-unified.md).** The forward plan is now TWO strikes — **Strike 1: NIGHTSHIFT criterion-5 (the live path)** — re-capture real messy chat turns under the step-0 B4 hook (now persists `ep.txt`/`ep.tok`), run the offline curator on them, prove distilled episodes land IN-DISTRIBUTION for the live W_c recall head (closes the documented live-0.084-vs-curated-9.858 gap; turns the curator from gated-GREEN-on-synthetic into a real organism loop). **Strike 2: the OOD diffusion kill-test (settle Phase 5)** — mint a held-out OOD novel corpus W_c never trained on, run the fast llama.cpp diffusion oracle vs the live W_c head on zero-shot recall + foreign-reject; if the diffusion oracle does NOT decisively beat W_c, the Phase-5 native diffusion judge lane (incl. N5b resident reservoir) is RETIRED on the record (W_c already beats the diffusion oracle IN-distribution 360/361 vs 95.6%; the only honest battlefield is OOD; N5b stays gated on the judge winning). **Standing:** the OKFS/SP-OKF + MEM-OKF content-addressed knowledge system is now the project's permanent auditability discipline (lookup-before-build pre-flight); the one carried-forward external item is the 2-physical-GPU byte-exact check. Honest tiers throughout — the curator is gated-GREEN-on-synthetic (criterion-5 live PENDING), the diffusion judge is UNPROVEN/in-drawer (the 95.6% is the external llama.cpp oracle's number; our native single-forward was falsified ~25%). Record `CONTRACT-NIGHTSHIFT-CURATOR.md` + `RFC-ORGANISM-unified.md` + `STATUS-MAP-2026-06-21.md`. Prior 2026-06-20: **B3-WC AUTONOMOUS-RECALL CAMPAIGN RESOLVED** — a learned W_c head does LIVE instance-level episodic recall on the served gemma-4-12B chat, with clean foreign-reject; engine `edc8079`; 360/361 recall + 50/50 reject offline (G-CHAT-B3-WC-DIV2 `87044d8`); corpus diversity was the binding constraint, every hand-designed signal a measured negative; B4 NIGHTSHIFT = next, pre-scoped. Record `CONTRACT-CHAT-FULLSTACK.md` + `SESSION-HANDOFF.md §0d`. Prior 2026-06-18: **XBAR UNIFIED onto the exact-integer O_K substrate** — the memory tier re-carried from float carriers onto Q(√−163) / the dual-prime negacyclic CRT-NTT; ten receipts, all GREEN or honest-negative; engine `0019b86→d2d7ceb`, all pushed. Container WINS: native bind 256/256 bit-identical + reduction-order-immune (`0019b86`), Frobenius integer Ring-2 store sub-ULP@24b (`dbe4103`/`d076797`), full organism loop native on real episodes (`15e7051`), period-6 rebase CLOSED (`d2d7ceb`). Content does NOT: four honest negatives — Dirichlet carriers (`d7d96fe`), Möbius-on-M (`1e70763`), entropy-on-codes (`e6d17bb`), T2-Möbius-on-weights (`ac76c8e`). **BOUNDARY THESIS:** the substrate's value is exact arithmetic (the algebraic container), never structure imposed on the high-entropy content. host-numpy→native Z_q/NTT port DONE. **SAME-DAY: the BYTE-EXACT FORWARD CLOSED GREEN on the real gemma-4-12B** — the four nonlinear islands (RMSNorm/softmax/GELU/RoPE) + attention now run exact-integer on the same dual-prime CRT-NTT behind default-off `SP_BYTEEXACT` (G-BYTEEXACT-FORWARD-12B: OFF 4.6665 byte-identical null floor / ON 4.6569 parity / run-to-run bit-identical), daemon-driven via the new L1 `sp_session_register_kvdecode_backend` verb (G-WIRE-CUDA-DECODE-GEMMA4 32/32 == oracle); byte-exact = auditability NOT compression; engine `69c0588` + submodule `d9d96f3`; the discrete/byte-exact-forward phase is COMPLETE, only the external 2-GPU check remains. **NEXT = T4 Frobenius π^k quantization of the 9.4 GB model WEIGHTS** (the validated lever; NOT Möbius). Prior amend 2026-06-17: **C2 MEMO CURATOR CLOSED** + **Ring-3 Path A CLOSED end-to-end** + **G-XBAR-ORGANISM step 1 GREEN**; **XBAR P3 CLOSED P3.0→P3.4** + **GNA "EAR" CLOSED on PHYSICAL SILICON**. Prior amend 2026-06-16: KAIROS axis CLOSED + 6h soak GREEN; KAI-2 CLOSED-BOUNDED; KAI-3 audio-port CLOSED GREEN. Human+agent keystone: [`CURRENT-STATE-OF-PROJECT.md`](../CURRENT-STATE-OF-PROJECT.md).)
...
**Today's forward edge (2026-06-21):**
- **>>> TODAY'S EDGE (2026-06-24) -- diffusion-judge perf SHIPPED (~2x, byte-exact) + a recall regression to chase + the Cola north-star.** (a) `SP_DG_SCRATCHREUSE` default-on ~1.46x (engine `e31c70d`) + `SP_DG_ASYNC` byte-exact pinned double-buffer prefetch of spillover experts, ~2x stacked (engine `2a1c830`, default-OFF); concurrency stress 1440/1440 MOECHK byte-exact OFF==ON. (b) **IMMEDIATE NEXT -- prefix-KV RESURRECTED (the real speedup).** Cola E1 (verified from `_diffgemma_reference/diffusion-gemma.cpp:43-54` + `ARCH-NOTES.md:40-52`): the reference prompt is causal-over-prompt and NEVER attends the canvas, so prompt K/V is canvas-invariant BY CONSTRUCTION, and the reference SHIPS a prefix-KV decode variant (`llm_graph_input_attn_diffusion_decode`, rectangular [P+C,C], forward only the canvas per step). Our 6.9e-4/NaN refutation was FALSE (fp-noise + the now-fixed OOB). PORT that decode variant = forward only 256 canvas tokens/step = the real diffusion-judge speedup; re-run `SP_DG_PREFIXKV_PROOF` first to confirm invariance now the OOB is fixed. NOTE: the earlier "recall regression / self-cond load-bearing" framing is WITHDRAWN -- the self-cond is correctly wired (`test_diffjudge_denoise.c:437` have_prev gate); the killed native run scored 8.3%, which is the NATIVE judge being weak (95.6% is the EXTERNAL oracle, native was always ~25% single-forward), and the "iterative denoise rescues the native judge" hypothesis is REFUTED. `SP_DG_ASYNC` is byte-exact + default-off, no regression blocks it. (c) **COLA residual** ([`DESIGN-COLA-DLM-MAPPING.md`](DESIGN-COLA-DLM-MAPPING.md) section 2, corrected): block-causal does NOT map to prefix-KV (the model is already prompt-causal); Cola value = latent diffusion / avoid the V=262144 vocab softmax (E3 feasibility, the bug class hit tonight). Detail: memory `project_perf_wholemachine.md` / `project_n6_prefixkv.md`.
- **▶▶▶ THE FORWARD PLAN — TWO STRIKES, in priority order (2026-06-21).** The recall organism is unified + recorded in [`RFC-ORGANISM-unified.md`](RFC-ORGANISM-unified.md) (served byte-exact O(1) 12B chat + live latent W_c RECALL selector + causal-ablation ADMISSION oracle + NIGHTSHIFT offline curator + MEM-OKF store + PoUW receipts). Honest tiers: the curator is **gated-GREEN-on-synthetic** (criterion-5 live PENDING); the diffusion judge **WON the OOD kill-test (Strike 2 RESOLVED, lattice `dda7ffa`: 94.4% vs W_c 28.3% @K8) → it is the zero-shot Stage-2 adjudicator and N5b is JUSTIFIED.**
  - **Strike 1 — NIGHTSHIFT criterion-5 (the live path).** Re-capture REAL messy chat turns under the step-0 B4 hook (which now persists `ep.txt`/`ep.tok`, engine `9ad7ede`), run `run_kairos_curator` (`SP_NIGHTSHIFT_OFFLINE`, engine `6107f3e`) on them, and prove the distilled episodes land IN-DISTRIBUTION for the live W_c recall head — closing the documented live-0.084-vs-curated-9.858 collapse (the B4 distributional-shape mismatch: live `read_global_k` ≠ curated `ep.k`; the ×1.96 norm patch did NOT fix it ⇒ shape not scale). This is what turns the curator from gated-GREEN-on-synthetic into a real organism loop. Gate criterion-5 of `G-NIGHTSHIFT-CURATOR`; record `CONTRACT-NIGHTSHIFT-CURATOR.md` §3.5/§7. *Kill: distilled live episodes still score out-of-band / break foreign-reject.*
  - **Strike 2 — the OOD diffusion kill-test (settle Phase 5). ✅ RESOLVED 2026-06-22 (lattice `dda7ffa`): DIFFUSION WINS → Phase-5 native lane (incl. N5b) JUSTIFIED.** At matched K=8 on a held-out OOD corpus W_c never trained on: **diffusion judge 94.4% recall / 98.0% reject vs W_c 28.3% / 96.9% = +66.1pp** (far outside the ±11% N=72 noise), clearing the pre-registered §4 kill-criterion (+10pp recall AND ≥96.9% reject). W_c beats the diffusion oracle IN-distribution (360/361 vs 95.6% on `_needle_corpus_div`) but is an in-distribution MEMORIZER (11.1% full-reg OOD) — the OOD battlefield went decisively to the diffusion judge, which becomes the **zero-shot Stage-2 adjudicator** over the W_c/LSH top-K. NB the 94.4% is the EXTERNAL llama.cpp PR-24423 26B oracle (Q4_K_M) as the capability proxy; our native single-forward judge was ~25% (`f8f76a5`) → **N5b** ([`DESIGN-diffgemma-n5b-reservoir.md`](DESIGN-diffgemma-n5b-reservoir.md)) + the iterative entropy-bound sampler is the build that makes our native judge match it. *(The first-pass "run INVALID / no tags" verdict was an analyst error: grepped the harness 40-char truncated result-line — a benign `embeddings required…overriding` init warning — not the full reply, which a verbose probe confirmed carries real tags + the `<channel>thought` reasoning. Corrected + receipted.)* Record `CONTRACT-DIFFJUDGE-OOD-H2H.md` §5 + `CONTRACT-NIGHTSHIFT-CURATOR.md` §4 Track 2 + `RFC-ORGANISM-unified.md` §5.
  - **Standing.** The OKFS/SP-OKF ([`SP-OKF-PROFILE.md`](SP-OKF-PROFILE.md)) + MEM-OKF ([`MEMORY-OKF-PROFILE.md`](MEMORY-OKF-PROFILE.md)) content-addressed knowledge system is the project's permanent auditability discipline (the **lookup-before-build pre-flight** is binding: `python tools/okf_mem.py lookup --root memory-okf <kw>` + grep BEFORE building anything). The one carried-forward EXTERNAL item is the **2-physical-GPU byte-exact check** (on-machine = run-to-run bit-identity + integer-reduction order-immunity as the proxy).
- **▶▶ B3-WC AUTONOMOUS-RECALL CAMPAIGN RESOLVED — a learned W_c head does LIVE instance-level episodic recall on the served gemma-4-12B chat (2026-06-20, engine `edc8079`, pushed).** The served `/v1/chat` (now coherent + byte-exact + O(1)-context + single-entry, all GREEN — `CONTRACT-CHAT-FULLSTACK`) selects the right stored episode for a query or refuses if none fits. The arc is a boundary-thesis win (negatives kept attached): ALL hand-designed relevance signals FAIL open-world — 6 verifiers + 4 Disposer signals (Yes/No, Δ-continuation, multi-token ΣΔLL, consensus) + cosine-qK + ΔLL-polarity, dominated by per-episode bias at N=3 wiki (`4dba6c8`/`2b623ab`/`acd7b3a`); ROOT CAUSE = wiki facts are PARAMETRIC so episodic dependency is unmeasurable (the corpus was the wall, B3-v10 `27038d3`); THE UNLOCK = a teacher-forced ablation knockout on a NOVEL needle (`SP_B3_SECRET` cudaMemset-ablates the secret's source KV rows, ΣΔLL with/without → novel −33.56 vs parametric −0.15, B3-v13 3-archetype matrix **TAU −8.0**, `15738c1`/`b6470cc`) — both a shippable gate and a perfect labeler (`7556d04`). Labeled a curator-minted diverse corpus (split-entropy minter, secret stated ONCE, 200 needles + 60 foreign, `52ec468`/`adfcb06`/`f4166c7`) → learned **W_c** (HD=512→r=32, **logsumexp-over-positions-then-mean-over-heads**, InfoNCE over [episodes + NULL/s0] + reject hinge). Corpus DIVERSITY was the binding constraint at every granularity (instance top-1 **34%→100%**, `mint_corpus_v2` `72d9c77`/`f62e6ef`); the int16 "degradation" was a wrong-reduction bug — **logsumexp-mean=361/361, int16==f32**. **G-CHAT-B3-WC-DIV2** (`87044d8`): 360/361 recall + 50/50 foreign reject, int16-exact, s0=+0.102. **G-CHAT-B3-WC-DEPLOY** (`edc8079`, LIVE): `recall.rs WcHead`/`wc_score` + `routes.rs SP_B3_WC` ((E+1)-NULL-argmax → replay winner @`SP_REPLAY_MTARGET=42` or clean prompt; default-off = null floor); matched → RECALL `ep_n_div_000` (9.858), foreign → NULL → clean "Paris." Launcher `run_console_recall.bat` (`eb6332d`, PMAX 4096 `82d77c8`) + console stop-button (`fc25b07`). **NEXT = B4 NIGHTSHIFT** (between-turn consolidation, DEFERRED, pre-scoped SESSION-HANDOFF §0d). Record `CONTRACT-CHAT-FULLSTACK.md`.
- **▶▶ XBAR UNIFIED onto the exact-integer O_K substrate (2026-06-18, engine `0019b86→d2d7ceb`, all pushed).** The memory tier was re-carried off generic float carriers onto Q(√−163) / the dual-prime negacyclic CRT-NTT (`core/ntt_crt`+`core/poly_ring`, already linked — **zero new linkage**, the `gemma4_kv_*` cache is pure f32). Ten receipts, all GREEN or honest-negative; receipts in engine `tests/fixtures/xbar_r3/` + `tests/fixtures/xbar_organism/`. **CONTAINER WINS:** G-R3-BIND-on-OK Leg A (`0019b86`) 256/256 bit-identical + ±1 carrier recall lossless + M byte-identical across 8 summation orders (float diverges 4.44e-15); G-R2-FROB integer Ring-2 store (`dbe4103`/`d076797`, Theorem-T4 form) sub-ULP@24b / lossless@16b, bit-width the lever (12b 2.86×/16b 2.0×), fidelity-proven NOT n=42-PPL; organism-native FFT-ripped-out (`1f0f6be`); G-XBAR-ORGANISM-FULL (`15e7051`) full loop native on real episodes (audio→discrete integer memory→KV out, autonomous); G-PERIOD6-REBASE (`d2d7ceb`) content-hash 8→6, decoy separation 154→129, all prior gates stand (the prior NEXT's period-6 rebase = CLOSED). **CONTENT DOES NOT — four honest negatives:** Dirichlet-character carriers (Leg B, `d7d96fe`, inert), Möbius-on-M (`1e70763`), entropy-on-codes (`e6d17bb`), T2-Möbius-on-real-weights (`ac76c8e`, worse than random — and T2 was a design proposal never validated, unlike T4 Frobenius). **BOUNDARY THESIS (keystone):** the substrate's value is exact arithmetic (the indestructible algebraic container), never number-theoretic structure imposed on the high-entropy content. The host-numpy→native Z_q/NTT port (long-deferred) is now DONE. Engine `tools/ring3/{g_r3_bind_ok.py,ok_bind.py,g_xbar_organism_full.py}` + `tools/curator/frob_episode.py`.
- **▶▶ BYTE-EXACT FORWARD CLOSED GREEN on the real gemma-4-12B (2026-06-18, engine `69c0588` + math-core submodule `d9d96f3`).** The *forward pass itself* now lives in the same exact-integer O_K container as the memory tier: the four nonlinear fp32 islands (RMSNorm / softmax / GELU-tanh / RoPE) + attention (Q·K / p·V dot + softmax) convert to exact-integer on the SAME dual-prime CRT-NTT (q1=1073738753, q2=1073732609, Garner inv 894602413, M≈2⁶⁰ fits u64 ⇒ NO `__int128` — device `__umul64hi`, 64-bit-split isqrt for RMS, device CORDIC for RoPE), behind default-off `SP_BYTEEXACT` (one-shot `gemma4_decode_cuda` byte-untouched = null floor). **byte-exact = EXACT ARITHMETIC / cross-machine determinism (AUDITABILITY), explicitly NOT compression** (the compression axis is convicted). Gates (real 12B `gemma4-12b-b1.sp-model`, RTX 2060): G-ISLANDS-Q-REF GREEN (host: RMS 5.8e-6 / softmax 1.3e-6 / GELU 2.8e-6 / RoPE 9.2e-6, order-immune) → G-BYTEEXACT-ISLANDS-CUDA GREEN (on-model layer-24 RMS 3.84e-5 / GELU 8.18e-7 / RoPE 9.62e-6, all < 1e-4) → **G-BYTEEXACT-FORWARD-12B GREEN** (OFF = PPL 4.6665 byte-identical null floor / ON = 4.6569 parity −0.21% n=42 / run-to-run BIT-IDENTICAL). The universal daemon drives the 12B prefill (G-WIRE-CUDA-GEMMA4) + token-by-token decode through the **NEW L1 verb `sp_session_register_kvdecode_backend`** (open/prefill/decode_step/rewind/position/close — the persistent-KV sibling of the prefill-only `sp_session_register_forward_backend`; G-WIRE-CUDA-DECODE-GEMMA4 32/32 bit-identical to the `gemma4_kv_decode` oracle, VRAM flat O(1)). The byte-exact LINEAR algebra was ALREADY in the crate `tools/sp_dsp_smoke` (Barrett/mod-q matmul/Garner/NTT, bit-exact-gated) — the new piece was the four nonlinear islands (`sp_islands_q_ref.rs` + math-core `core/exact_islands/`). ABI grew append-only (`PPT-LAT-L1-ABI-v0` §6b); OK_Q4B loader reconciled decision-B (`PPT-LAT-SP-MODEL-v0` §6.5). **The discrete/byte-exact-forward phase is COMPLETE; the ONLY remaining item is the EXTERNAL 2-physical-GPU bit-identical check** (the run-to-run bit-identity + integer-reduction order-immunity are the on-one-machine proxy). Design + gates: `CONTRACT-BYTEEXACT-forward.md` §5.1/§5.2/§7/§8; public Papers 19/20/21 + `LEDGER.md` (`19f3d8c`).
- **▶ C2 MEMO CURATOR CLOSED end-to-end (Steps 1–3.1 + #222 + G-XBAR-ORGANISM step 1, all GREEN, 2026-06-17).** The autonomous Ring-2 recall loop is fully operational: index (registry + 256-bit LSH hash, discrete bit-collision resolver, TAU_BITS=168, r-sweep proved r=32 collapses/r=256 ships), cue (key-based, key-centroid centroid, empty-registry→NULL), orchestrator (one-shot loop, Option A), G-MEMO-NULL (PPL 4.6665 bit-identical off), G-MEMO-LOOP 12B (ACCEPT +0.000% → PROMOTE, REJECT zeroed +40106.6% → FLAG+DISCARD). #222 ports SP_REPLAY into the persistent `gemma4_kv_*` ABI (`gemma4_kv_replay`) + O(1) bit-exact rewind; G-222 GREEN E2B+12B; G-222-WRAP GREEN (SWA-ring journal-backed). G-XBAR-ORGANISM step 1: EAR→Ring-2 write seam proven (ep_audio [48,114,512] uniform-512, sig separates self 211/256 margin +79, SP_REPLAY loads+injects clean, +1989% is foreign-by-design). Contracts: `CONTRACT-XBAR-C2-memo-curator-loop.md`; engine runs `6dd87b9` / `3ea0587` / `627dfad` / `b4b037a` / `24071bc` / `6600cf4`. NOTE: PERIOD=8 vs true 12B SWA period 6 — a correctness tidy-up, all prior gates STAND.
- **▶ Ring-3 Path A CLOSED end-to-end — parameter-free, zero training budget (2026-06-17).** The gist/neocortical tier above closed Ring-2: R3.1 G-R3-BIND (VSA/HRR superposition, addr seeded from real C2 256-bit sigs, recall@1=1.0 to N=32 @ D=1024, margins +0.586/+0.568); R3.2 G-R3-LOSS (consolidation loss = step function: hit +0.000% / miss +8.04% → gate-caught + O(1)-rewind; budget ≤32 episodes/vector; 71µs unbind); R3.3 G-R3-DUALROUTE (clean hit + decoy scan + null parity, survives wrong candidates, degrade-safe); R3.4 G-R3-NIGHTSHIFT (idle-loop state machine: bind→shadow-gate→promote+evict-to-Optane→seal; 40 episodes → 349.8 MB KV → 16.3 KB Ring-3 index; D=128 gate-driven seal proves seal is the math). Deferred: Z_q/NTT engine port, G-R3-PROV, Path B adapter. Contract: `CONTRACT-XBAR-R3-consolidation.md`.
- **▶ XBAR / NIGHTSHIFT COMPLETE end-to-end (2026-06-18).** Mechanisms all GREEN (P1→P3.4 + C2 Steps 1–3.1 + #222 + Ring-3 Path A R3.1→R3.4 + organism + native O_K bind) AND the **native-C `core/`-resident port is CLOSED**: `core/ring3/` (`ring3.c` + `include/sp/ring3.h`) ports the host-Python VSA layer + NIGHTSHIFT state machine onto native `sp_pr_mul`; gate **T_RING3_NATIVE 42/42** (bind/unbind/superpose BIT-IDENTICAL to the Python `ok_bind` reference; NIGHTSHIFT reproduced [32,8]@D=1024 + gate-before-cap@D=128; order-immune); carriers unified to splitmix64 (no Python recall regression). Math-core `e0fccd3`, engine submodule `7b992d2`. The resident consolidation loop no longer needs Python. REMAINING XBAR items are OPTIONAL / deferred-by-choice: the **N1 unattended persistence soak** (deployment endurance — deferrable like the KAIROS ≥24h gate), **G-R3-PROV** (secondary enhancer, adopt only if Δppl<0), **Path B** (learned adapter — only if Path A shortlist measured insufficient + budget green).
- **▶ NEXT = the two strikes above (Strike 1 NIGHTSHIFT criterion-5 → Strike 2 OOD diffusion kill-test); see the top forward-edge box.** B4 NIGHTSHIFT was realigned from a live between-turn hook into the **OFFLINE curator** (`run_kairos_curator`, gated-GREEN-on-synthetic) — Strike 1 is its criterion-5 live close, which is ALSO the measured route around the live B4 distributional-shape mismatch. The major campaigns are all closed — XBAR/NIGHTSHIFT mechanisms, the byte-exact forward, KAIROS, GNA EAR, and the full-stack 12B chat incl. B3-WC autonomous recall. **T4 Frobenius π^k on the WEIGHTS is NOT next — it is CONVICTED:** the incoherence-rotation / Hadamard-fold compression was ruled REDUNDANT vs the per-32-block OK_Q4B (G-WEIGHT-FOLD-ORACLE `8ae8825`, "do NOT build"; the 7th structure-on-content negative = the boundary thesis), and the byte-exact angle (OK_Q8 per-tensor π^k) was absorbed into the now-closed byte-exact forward (the dp4a integer accumulate already makes OK_Q4B exact). The diffusion lane is NOT an open candidate — it is gated behind Strike 2's kill-test (retired unless it beats W_c OOD). The one carried-forward external item is the 2-physical-GPU byte-exact check (needs a 2nd machine). *(Prior NEXTs — period-6 rebase, full organism loop, host-numpy→native VSA port, B4-as-live-hook — all CLOSED or realigned above.)*
- **XBAR P3 CLOSED end-to-end (P3.0→P3.4, 2026-06-17).** P3.3 SP_REPLAY `G-P3-SHARED` 3-leg PASS 12B+E2B (intact bit-exact, zeroed diverges 12/12); P3.4 G-P3-PPL +1.38% < 2% (4.6665→4.7311, n=42 single-chunk, deterministic; larger-N = the hardening lever). **GNA "EAR" line CLOSED on PHYSICAL SILICON (2026-06-17):** real speech → 12B pivot 7/8 (CTC 0.44→0.868); POT GNA-native i16 = 0.877 full FP32 recovery; GNA_HW on Intel GNA 2.0 = 0.877 == emu == FP32. Audio pivot DONE; project pivoted back to XBAR.
- **KAIROS time/agency axis CLOSED GREEN (2026-06-16; 6h soak GREEN; formal ≥24h gate un-pursued by choice, NOT failed).** KAI-1/1b/1c + `run_kairos_metal` crucible; KAI-2 CLOSED-BOUNDED; KAI-3 CLOSED GREEN. Contract `CONTRACT-KAIROS-K0-K1.md` §5.5-5.9/§6.6/§7.3-7.6.
- **Phase C alloc-shrink CLOSED GREEN (2026-06-14):** O(1) realized (N=8192 vs 16384 VRAM flat ~50 MiB, engine `33ac632`) + C-c NIAH retention at depths 10/50/90% (engine `3218d73`). §P3.2-b-2b learned-LSH 8× = +0.47% PPL. KV cache DECOUPLED from context length (both terms O(1) in P).
