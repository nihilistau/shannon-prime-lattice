...
> **AMENDMENT (2026-07-02, under the ADR-002 regime): Path B, if ever opened, is a GIST HEAD on the decide spine — NOT a standalone adapter model.** Consolidation (select spans, compress n→k, emit a content-address key) is a **DECIDE** operation, and ADR-002 puts deciders as cheap heads on the resident body, never separate models. So "Path B = P2.b learned adapter model" collapses to a **gist head in the W_c / Memory-Head family on the global-K/L5 retrieval substrate** (where the fact signal is proven — L5-cosine 86.89%; draft-body→global-K bridge = fallback entry). It must be a **recognition/shortlist** head (emit a key; Ring-2 exact bytes carry fidelity via retrieve-and-verify), never reconstructive — forced by both P2.b's verdict (span→k generation dead; recognition top-5 0.77) and the never-fuse rule. This reduces Path B from "train + host an adapter model + operator budget" to "a head + a distillation gate on the existing spine," still deferred-by-evidence (only if Path A's shortlist is *measured* insufficient — the 16×–32× budget-ladder MISS). Full reasoning: [PPT-LAT-ADR-LATENT-NATIVE-UNIFICATION.md](PPT-LAT-ADR-LATENT-NATIVE-UNIFICATION.md) §4 (Consolidation graduation).
