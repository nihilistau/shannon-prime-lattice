...
**Update 2026-06-09 — XBAR, the auditable latent crossbar (STATE §5.14): token-free inter-model memory.** A new architecture lane atop the proven engine: two models share the cyclotomic rings and communicate through **latent state, not tokens**, every write receipted/gated/rewindable. **Citable (public LEDGER X-R1):** a 12B's generation is steered by **direct KV-cache transplant with no tokens** — 15/15 lexical incorporation across a 5×3 matrix, double-dissociation selectivity, a measured dose-response (≈4% attention mass bends ranks, six contiguous rows bend the words), gold-instrument-verified coherence. The compaction adapter (P2.b) Phase-0 inversion ran on cloud A6000s and **measured 6→2 span compression on the real bf16 12B** (recovery/manifold Pareto mapped twice); its free-generation parity gate was honestly convicted as unusable (greedy decode loops on the it-model) and the operating point moved into training as a λ-selection. Locally the **curator immune system runs as committed code**: the transactional shadow ring (clone/gate/atomic-promote/rewind/receipts) and episode-persistence with bit-exact router re-projection. Architecture grew a four-tier hierarchy (Ring 3 = consolidated "neocortical" store under an irreversible loss gate). Docs: `papers/RFC-XBAR-auditable-latent-crossbar.md` + `CONTRACT-XBAR-*`; cloud mechanism in `papers/RUNBOOK-cloud-compute.md`.

**Update 2026-06-08 — THE GEMMA-4 CAMPAIGN CLOSED (STATE §5.13): sovereign quantization pipeline + the citable speed/quality point.** A from-scratch reference forward measured gemma-4-12B's TRUE wikitext PPL at **4.6776** and convicted the entire GGUF ecosystem (192–506, including post-fix rebuilds; llama.cpp's forward exonerated — the artifacts are broken). The answer: **Safetensors Direct** (`sp_transcode --st`, weight values from the checkpoint only) + the **OK_Q4B** block-scaled format (arena layout v2; recipe B1 chosen by 6-recipe simulation) → a 9.4 GB artifact whose built PPL matched the simulation to four decimals (5.1259) and whose GPU kernel landed at 5.1160. **Citable (public LEDGER 06-R10): Gemma-4-12B at 26.1 tok/s and PPL 5.12 on the RTX 2060 12GB** — llama.cpp: 31.29 tok/s at PPL 192–506; SP engine bandwidth +18%; the earlier 34.2 headline is retired with its quality-failed artifact. Papers 04/05/06 written; community tutorial `GEMMA4-QUANT-FIX.md` in the public repo. Full record: `CONTRACT-SPEED`; receipts `tests/gemma4_gold/`.
